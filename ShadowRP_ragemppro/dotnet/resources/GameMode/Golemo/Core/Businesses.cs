...
namespace Golemo.Core
{
    class BusinessManager : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                foreach (DataRow Row in result.Rows)
                {
                    ...
                    if (data.Type == 1 && ElectroGas.ContainsKey(id))
                    {
                        List<Vector3> vec1 = ElectroGas[id];

                        foreach (Vector3 vec in vec1)
                        {
                            ColShape shape = NAPI.ColShape.CreateCylinderColShape(vec, 3, 3, 0);

                            shape.OnEntityEnterColShape += (s, entity) =>
                            {
                                try
                                {
                                    NAPI.Data.SetEntityData(entity, "INTERACTIONCHECK", 520);
        ...
        public static List<VehicleHash> ElectroCar = new List<VehicleHash>()
        {

              (VehicleHash)NAPI.Util.GetHashKey("v250"),
              (VehicleHash)NAPI.Util.GetHashKey("msprinter"),
              (VehicleHash)NAPI.Util.GetHashKey("i8"),
              (VehicleHash)NAPI.Util.GetHashKey("m8"),
              (VehicleHash)NAPI.Util.GetHashKey("taycan21"),
              (VehicleHash)NAPI.Util.GetHashKey("vision6"),
              (VehicleHash)NAPI.Util.GetHashKey("vision"),
              (VehicleHash)NAPI.Util.GetHashKey("mvisiongt"),
              (VehicleHash)NAPI.Util.GetHashKey("p90d"),
         };

        public static Dictionary<int, List<Vector3>> ElectroGas = new Dictionary<int, List<Vector3>>()
        {
            {1,new List<Vector3>{ new Vector3(-2061.3486, -306.99942, 12.02727)}},
        };
        public static List<Vector3> ElectroGasRotation = new List<Vector3>()
        {
            { new Vector3(0,0,90) },

        };
        public static void OpenElectroGasMenu(Player player)
        {
            if (!player.IsInVehicle) return;
            Vehicle vehicle = player.Vehicle;
            if (vehicle == null) return; //check
            if (player.VehicleSeat != 0) return;
            if (!ElectroCar.Contains((VehicleHash)vehicle.Model))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Это заправка для электрокаров", 3000);
                return;
            }
            Trigger.ClientEvent(player, "openInput", "Введите сколько вам нужно заправить", "Количество", 7, "electrogas");
        ...
        public static List<string> CarsNamesDonate = new List<string>()
        {
            "chiron19",
            "cullinan",
            "msprinter",
        ...
        public static Dictionary<string, int> ProductsOrderPrice = new Dictionary<string, int>()
        {
            ...
            { "CombatPistol", 7400 },
            { "HeavyPistol", 9000 },
            { "DoubleAction", 2500000 },
            { "Revolver", 187000 },
            { "BullpupShotgun", 18000 },
            { "CombatPDW", 27000 },
            { "MachinePistol", 16000 },
            { "Hammer", 600 },
            { "Crowbar", 750 },
        ...
        public static List<Product> fillProductList(int type)
        {
            ...
            switch (type)
            {
                ...
                case 1:
                    ...
                    products_list.Add(new Product(ProductsOrderPrice["Электричество"], 0, 0, "Электричество", false));
                    break;
                case 2:
        ...
        public static int GetBuyingItemType(string name)
        {
            ...
            switch (name)
            {
                ...
                case "Банка Sprite":
                    ...
                    break;
                case "Нож":
                    type = (int)ItemType.Knife;
                    break;
                case "Розовый дилдо":
                    type = (int)ItemType.Dildo;
                    break;
                case "Черное дилдо":
                    type = (int)ItemType.DildoBl;
                    break;
                case "Красное дилдо":
                    type = (int)ItemType.DildoRed;
                    break;
                case "Рация":
                    type = (int)ItemType.Walkie;
                    break;
                case "Аптечка":
                    type = (int)ItemType.Apteka;
                    break;
                case "Бинт":
                    type = (int)ItemType.Bint;
                    break;
                case "Бинокль":
                    type = (int)ItemType.Binocular;
                    break;
                case "Удочка":
                ...
                case "Наживка":
                    ...
                    break;
                case "Стяжки":
                    type = (int)ItemType.Cuffs;
                    break;
                case "Мешок":
                    type = (int)ItemType.Pocket;
                    break;
                case "Балончик":
                    type = (int)ItemType.Balon;
                    break;
                case "Бонг":
                    type = (int)ItemType.Bong;
                    break;
                case "Бонг цветной":
                    type = (int)ItemType.RGBBong;
                    break;
                case "Зажигалка":
                    type = (int)ItemType.Lighter;
                    break;
                case "Бумбокс":
                    type = (int)ItemType.Boombox;
                    break;
            }
            ...
            try
            {
                ...
                switch (type)
                {
                    ...
                    case 9:
                        ...
                        break;
                    case 10:
                        tempPrice = Customization.Decals[Main.Players[player].Gender].FirstOrDefault(l => l.Variation == variation).Price;
                        break;
                }
                var price = Convert.ToInt32((tempPrice / 100.0) * prod.Price);
                ...
                switch (type)
                {
                    ...
                    case 9:
                        ...
                        break;
                    case 10:
                        Customization.AddClothes(player, ItemType.Decals, variation, texture);
                        break;
                }
        ...
        public static void fillCar(Player player, int lvl)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Этот вид топлива не для вашего авто", 3000);
                    return;
                }
                if (VehicleManager.DieselVehicles.Contains((VehicleHash)vehicle.Model))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Этот вид топлива не для вашего авто", 3000);
                    return;
                }
                if (VehicleManager.PremiumVehicles.Contains((VehicleHash)vehicle.Model))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Этот вид топлива не для вашего авто", 3000);
                    return;
                }
        ...
        [RemoteEvent("petrol_diesel")]
        public static void fillCarDiesel(Player player, int lvl)
        {
            try
            {
                if (player == null || !Main.Players.ContainsKey(player)) return;
        ...
        [RemoteEvent("petrol_premium")]
        public static void fillCarPremium(Player player, int lvl)
        {
            try
            {
                if (player == null || !Main.Players.ContainsKey(player)) return;
        ...
        public static void buyBusinessCommand(Player player)
        {
            ...
            if (biz.Type == 2)
            {
                Notify.Error(player, "Данный бизнес недоступен для покупки");
                return;
            }
        ...
        public static void OpenBizSettingMenu(Player player, string product)
        {
            ...
            foreach (var p in biz.Products)
                if (p.Name == product)
                {
                    ...
                    if (p.Name != "Электричество")
                    {
                        var price1 = (product == "Патроны") ? 4 : ProductsOrderPrice[product];
                        menuItem = new Menu.Item("order", Menu.MenuItem.Button);
                        menuItem.Text = $"Заказать: {price1}$/шт";
                        menu.Add(menuItem);
        ...
        // this create page in biz, all bizness who have shop menu. have different pages
        /*

            { BIZTYPE, new Dictionary<string, List<int>>  {
                {"NAME", new List<int> { INDEX PRODUCT } },
                {"NAME", new List<int> { INDEX PRODUCT } },
                {"NAME", new List<int> { INDEX PRODUCT } },
            } }

            { 0, new Dictionary<string, List<int>>  {
                    {"Eat", new List<int> { 0, 1 } },
                    {"Fastfood", new List<int> { 2 } },
            } }

        */
        static Dictionary<int, Dictionary<string, List<int>>> Pages = new Dictionary<int, Dictionary<string, List<int>>>
        {
            // the expample
            { 0, new Dictionary<string, List<int>>  {
                {"Еда", new List<int> { 5, 6 } },
                {"Электроника", new List<int> { 1, 7 } },
                {"Прочее", new List<int> { 2, 3, 4, 8, 9, 10 } },
            } }
        };
        ...
        public static void OpenPetrolMenu(Player player)
        {
            ...
            Trigger.ClientEvent(player, "openPetrol", prod.Price, fuel);
        }
    ...
    public class Business
    {
        ...
        public Business(int id, string owner, int sellPrice, int type, List<Product> products, Vector3 enterPoint, Vector3 unloadPoint, int bankID, int mafia, List<Order> orders)
        {
            ...
            blip = NAPI.Blip.CreateBlip(Convert.ToUInt32(BusinessManager.BlipByType[Type]), EnterPoint, 0.8f, Convert.ToByte(BusinessManager.BlipColorByType[Type]), Main.StringToU16(BusinessManager.BusinessTypeNames[Type]), 255, 0, true, 0, 0);
            ...
            switch (Type)
            {
                ...
                case 15:
                case 16:
                    marker = NAPI.Marker.CreateMarker(27, EnterPoint + new Vector3(0, 0, 0.15), new Vector3(), new Vector3(), 1f, new Color(50, 205, 50, 0), false, 0);
                    break;
                case 2:
                case 3:
                case 4:
               case 17:
                    marker = NAPI.Marker.CreateMarker(36, EnterPoint + new Vector3(0, 0, 1), new Vector3(), new Vector3(), 0.7f, new Color(50, 205, 50, 0), false, 0);
                    break;
                case 5:
                    marker = NAPI.Marker.CreateMarker(37, EnterPoint + new Vector3(0, 0, 1), new Vector3(), new Vector3(), 0.7f, new Color(50, 205, 50, 0), false, 0);
                    break;
                case 11:
                    marker = NAPI.Marker.CreateMarker(31, EnterPoint + new Vector3(0, 0, 1), new Vector3(), new Vector3(), 0.7f, new Color(254, 94, 0, 0), false, 0);
                    break;
                case 12:
                case 13:
                    marker = NAPI.Marker.CreateMarker(26, EnterPoint + new Vector3(0, 0, 0.15), new Vector3(), new Vector3(), 3f, new Color(50, 205, 50), false, 0);
                    break;
            }
        }
