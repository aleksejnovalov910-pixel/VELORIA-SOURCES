    ...
    public class AutoSchool : Script
    {
        ...
        public static void StartExam_Server(Player player, int id)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "У Вас недостаточно денег, чтобы купить данную лицензию", 3000);
                    return;
                }
                switch (id)
                {
                    ...
                    case 3:
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Вы успешно купили лицензию на вертолет", 3000);
                        break;
                    case 4:
                        Main.Players[player].Licenses[5] = true;
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Вы успешно купили лицензию на самолет", 3000);
                        break;
                }
                ...
                if (vehicle != null)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Езжайте по точкам", 3000);
                }
            }
        ...
        public void Event_OnPlayerExitVehicle(Player player, Vehicle vehicle)
        {
            try
            {
                if (player.HasData("SCHOOLVEH") && player.GetData<Vehicle>("SCHOOLVEH") == vehicle)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Если вы не сядете в машину в течение 60 секунд, то провалите экзамен", 3000);
                    return;
                }
            }
        ...
        private void timer_exitVehicle(Player player)
        {
            NAPI.Task.Run(() =>
            {
                try
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Вы провалили экзмен", 3000);
                }
