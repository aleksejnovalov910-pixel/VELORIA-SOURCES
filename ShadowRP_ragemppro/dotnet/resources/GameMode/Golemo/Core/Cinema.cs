...
class Cinema : Script
{
    ...
    public static bool cinema_open = false;

    private static nLog RLog = new nLog("Cinema");

    public static Vector3 cinema_enter = new Vector3(-1423.4061279296875, -215.29371643066406, 46.500423431396484);
    public static Vector3 cinema_exit_here = new Vector3(-1424.0640869140625, -210.75747680664062, 46.500423431396484);
    ...
    public static void OnResourceStart()
    {
        try
        {
            var shapecinema = NAPI.ColShape.CreateCylinderColShape(cinema_enter, 2, 3, 0);
            shapecinema.OnEntityEnterColShape += (shape, player) =>
            {
                try
                {
                    player.SetData("INTERACTIONCHECK", 903);
                }
                catch (Exception ex) { Console.WriteLine("shape.OnEntityEnterColShape: " + ex.Message); }
            };
            shapecinema.OnEntityExitColShape += (shape, player) =>
            {
                try
                {
    ...
    { 
        ...
        Blip temp_blip = null;
        temp_blip = NAPI.Blip.CreateBlip(cinema_enter);
        ...
        NAPI.Marker.CreateMarker(1, cinema_enter - new Vector3(0, 0,1.0), new Vector3(), new Vector3(), 0.8f, new Color(67, 140, 239, 200));
    ...
    {
        player.Position = new Vector3(-1423.314, -210.4525, 46.50041);
