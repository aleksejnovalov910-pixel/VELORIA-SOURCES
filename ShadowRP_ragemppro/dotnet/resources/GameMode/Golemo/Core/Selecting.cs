            ...
            try
            {
                ...
                switch (index)
                {
                    ...
                    case 6:
                        ...
                        bool access2 = (items.FindIndex(i => i.Type == ItemType.Number) != -1);
                        if (access2)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Недостаточно места в инвентаре", 3000);
                            return;
                        }
                        if (vehicle.HasData("ACCESS") && vehicle.GetData<string>("ACCESS") == "FRACTION" || vehicle.NumberPlate == "ADMIN")
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Это не ваша машина.", 3000);
                            return;
                        }
                        var numbers = vehicle.NumberPlate;
                        var holder = VehicleManager.Vehicles[numbers].Holder;
                        if (VehicleManager.Vehicles[numbers].Сasco == 0)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "На этом ТС отсутствует номер", 3000);
                            return;
                        }
