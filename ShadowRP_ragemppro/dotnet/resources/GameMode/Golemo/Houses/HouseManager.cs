...
namespace Golemo.Houses
{
    ...
    class House
    {
        ...
        public void SetOwner(Player player)
        {
            ...
            if (player != null)
            {
                ...
                Trigger.ClientEvent(player, "createGarageBlip", HouseManager.Houses[ID].Position);
        ...
        public void RemovePlayer(Player player, bool exit = true)
        {
            ...
            if (player.HasSharedData("TakeHijackingItem"))
            {
                NAPI.Task.Run(() =>
                {
                    ...
                    catch (Exception e) { Console.Write("NAPI.Task.Run: " + e.ToString(), nLog.Type.Error); }
                }, 50);
            }
        }
    ...
    class HouseManager : Script
    {
                ...
                case 6:
                    {
                        ...
                        if (house == null) return;
                        Scripts.HijackingHouse.HijackingHouseData data = Scripts.HijackingHouse.HijackingHouseData.HijackingHouseDic.FirstOrDefault(i => i.Player == player);
                        if (data != null)
                        {
                            if (data.House == house.ID && data.TypeHouse == house.Type)
                            {
                                if (player.HasData("TakeHijackingItem:Server"))
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы не можете вернуться в дом с частью его интерьера.", 3000);
                                    return;
                                }
                                if (house.Locked)
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Дом закрыт, взломайте его с помощью отмычки", 3000);
                                else
        ...
        {
            House house = Houses.FirstOrDefault(h => h.ID == player.GetData<int>("HOUSEID"));
            Trigger.ClientEvent(player, "HouseMenuBuy", id, house.Owner, HouseTypeList[house.Type].Name, house.Locked, house.Price, GarageManager.GarageTypes[GarageManager.Garages[house.GarageID].Type].MaxCars, MaxRoommates[house.Type], house.Type);
        }
        public static void OpenHouseMenuInform(Player player, int id)
        {
            ...
            {
                Trigger.ClientEvent(player, "HouseMenu", id, house.Owner, HouseTypeList[house.Type].Name, house.Locked, house.Price, GarageManager.GarageTypes[GarageManager.Garages[house.GarageID].Type].MaxCars, MaxRoommates[house.Type], house.Type);
                return;
            }
            Trigger.ClientEvent(player, "HouseMenu1", id, house.Owner, HouseTypeList[house.Type].Name, house.Locked, house.Price, GarageManager.GarageTypes[GarageManager.Garages[house.GarageID].Type].MaxCars, MaxRoommates[house.Type], house.Type);
        }
        ...
        {
            House house = Houses.FirstOrDefault(h => h.ID == act);
            if (house.Locked && player.GetData<int>("HOUSEHEIST") == GetHouse(player).ID)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Двери закрыты вам нужно их взломать", 3000);
                return;
            }
            if (house.Locked && player.GetData<int>("HOUSEHEIST") == -1)
            {
                var playerHouse = GetHouse(player);
        ...
        private static void callback_housebuy(Player player, int act)
        {
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Поздравляем! Вы купили дом #{house.ID} за {house.Price}$", 3000);
            ...
            switch (item.ID)
            {
                ...
                case "spawnmycar":
                    ...
                    if (!check)
                    {
                        if (number != null)
                        {
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы вызвали транспорт , ожидайте его в течений одной секунды", 3000);
                            NAPI.Task.Run(() =>
                            {
                                var pos = player.Position; garage.SpawnCarAtPosition(player, number, pos - new Vector3(3, 0, 0), player.Rotation);
                            }, delayTime: 100);
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Кхм... Эта машина не стоит в гараже", 3000);
                        return;
                    }
                    return;
        ...
        public static int GetDimension(House house)
        {
            return house.Dimension;
        }
        #endregion
    }
}
