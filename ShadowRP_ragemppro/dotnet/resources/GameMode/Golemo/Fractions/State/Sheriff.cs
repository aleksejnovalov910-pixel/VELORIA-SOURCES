...
namespace Golemo.Fractions
{
    class Sheriff : Script
    {
        ...
        #region menu
        public static void OpenSheriffClothesMenu(Player player)
        {
            Menu menu = new Menu("Sheriffclothes", false, false);
            menu.Callback = callback_Sheriffclothes;
        ...
        private static void callback_Sheriffclothes(Player client, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                case "combat1":
                    MenuManager.Close(client);
                    OpenSheriffCombatMenu(client);
                    return;
                case "payday":
        ...
        public static void OpenSheriffCombatMenu(Player player)
        {
            Menu menu = new Menu("Sheriffcombat", false, false);
            menu.Callback = callback_Sheriffcombat;

            Menu.Item menuItem = new Menu.Item("header", Menu.MenuItem.Header);
            menuItem.Text = "Форма";
            menu.Add(menuItem);

            menuItem = new Menu.Item("sher1", Menu.MenuItem.Button);
        ...
        private static void callback_Sheriffcombat(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            if (item.ID == "back")
            {
            ...
            switch (item.ID)
            {
                ...
                case "takeoff":
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы переоделись в рабочую форму", 3000);
                    return;
            }
        }
        #endregion
            ...
            try
            {
                switch (index)
                {
                    ...
                    case 8: // smg ammo
                        ...
                        Fractions.Manager.giveAmmo(client, ItemType.PistolAmmo2, 30);
                        return;
                    case 9: // shotgun ammo
