...
namespace Golemo.Fractions
{
    ...
    {
        private static nLog Log = new nLog("KillerBuro");

        private static Dictionary<int, ColShape> Cols = new Dictionary<int, ColShape>();
        public static List<Vector3> Coords = new List<Vector3>
        {
            new Vector3(-1146.5703, 4940.6704, -2021.14868), // Колшэйп входа в бункер   0
            new Vector3(2154.663, 2920.9873, -603.022495), // Колшэйп изнутри этажа, чтобы вернуться назад       1
            new Vector3(2113.3254, 2928.186, -603.521942), // guns    2
            new Vector3(2106.29, 2931.6814, -603.521942),  // open stock     3
             new Vector3(2111.3254, 2928.186, -630.521942),  // open stock     3
        };
        ...
        public static void interactPressed(Player player, int interact)
        {
            switch (interact)
            {
                ...
                case 855:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не состоите в KillerBuro", 3000);
                        return;
                    }
                    if (interact == 822) NAPI.Entity.SetEntityPosition(player, Coords[1] + new Vector3(0, 0, 1.12));
                    else if (interact == 855) NAPI.Entity.SetEntityPosition(player, Coords[0] + new Vector3(0, 0, 1.12));
                    return;
                case 105:
                    if (Main.Players[player].FractionID != 17)
                    {
        ...
        private static void callback_merycombat(Player client, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            {
                Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы не состоите в KillerBuro", 3000);
                return;
            }
            if (!client.GetData<bool>("ON_DUTY"))
            {
