...
namespace Golemo.Fractions
{
    class FractionCommands : Script
    {
        ...
        public static void giveArmyLic(Player player, Player target)
        {
            ...
            Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы выдали армейский билет игроку ({target.Value})", 3000);
            Trigger.ClientEvent(player, "openDialog", "ARMY_LIC", $"Игрок ({player.Name}) передал вам армейский билет. Для получения нажмите на кнопку ДА");
            target.SetData("SELLER", player);
        }
        public static void acceptArmyLic(Player player)
        {
            if (!Main.Players.ContainsKey(player)) return;
            Player seller = player.GetData<Player>("SELLER");
            if (!Main.Players.ContainsKey(seller)) return;
            ...
            Dashboard.sendStats(player);
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы получили Армейский Билет", 3000);
            Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы выдали Армейский Билет ({player.Name})", 3000);
        }
        ...
        public static void healTarget(Player player, Player target, int price)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы должны установить цену до 100$", 3000);
                return;
            }
            target.SetData("SELLER", player);
            target.SetData("PRICE", price);
            if (price != 0)
            {
                Trigger.ClientEvent(target, "openDialog", "PAY_HEAL", $"Доктор ({player.Value}) предложил лечение за ${price}");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы предложили лечение игроку ({target.Value}) за {price}$", 3000);
            }
            else
            {
                Trigger.ClientEvent(target, "openDialog", "PAY_HEAL", $"Доктор ({player.Value}) предложил вам вылечиться");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы предложили бесплатое лечение игроку ({target.Value})", 3000);
            }
            return;
        }
