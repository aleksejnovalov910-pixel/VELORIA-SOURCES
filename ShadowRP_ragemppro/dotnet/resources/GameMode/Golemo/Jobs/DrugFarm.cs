...
namespace Golemo.Jobs
{
    class DrugFarm : Script
    {
        ...
        private static int _drugsPayment = 1000;  //статичная цена
        ...
        public static void StartWorkDay2(Player player)
        {
            ...
            if (player.GetData<bool>("ON_WORK"))
            {
                ...
                Golemo.Trigger.ClientEvent(player, "deleteCheckpoint", 15);
                Golemo.Trigger.ClientEvent(player, "deleteWorkBlip");
            ...
            else
            {
                ...
                Golemo.Trigger.ClientEvent (player, "createCheckpoint", 15, 1, Checkpoints2[check].Position, 1, 0, 255, 0, 0);
                Golemo.Trigger.ClientEvent(player, "createWorkBlip", Checkpoints2[check].Position);
            ...
            switch (id)
            {
                ...
                case 382:
                    try
                    {
                        ...
                        Golemo.GUI.Dashboard.sendItems(client);
                        int payment = Convert.ToInt32((drugs.Count * _drugsPayment * _drugsMultiplier)); // количество * fix-price * коеффициент
                        Golemo.MoneySystem.Wallet.Change(client, payment);
        ...
        private static void PlayerEnterCheckpoint2(ColShape shape, Player player)
        {
            try
            {
                ...
                Golemo.Trigger.ClientEvent(player, "DrugOpenMenu2");
            }
            catch (Exception e)
            {
        ...
        private static void DrugStopWork(Player player, int count)
        {
            try
            {
                if (player != null && Main.Players.ContainsKey(player))
                {
                    ...
                    Golemo.MoneySystem.Wallet.Change(player, 150000);
                    ...
                    Golemo.Trigger.ClientEvent(player, "createCheckpoint", 15, 1, Checkpoints2[nextCheck].Position, 1,
                        0, 255, 0, 0);
                    Golemo.Trigger.ClientEvent(player, "createWorkBlip", Checkpoints2[nextCheck].Position);
                }
            }
            catch
            {
