...
export interface colshapeHandleOptions {
    /** Измерение */
    dimension?: number;
    /** Радиус маркера (vizual) */
    radius?: number;
    /** Радиус колшейпа (logic de interacțiune) */
    ...
    /** RGBA цвет */
    color?: RGBA,
    /** Modelul unui obiect custom care va fi plasat în locul markerului */
    customProp?: string;
...
export class colshapeHandleBase extends GameVisualElement {
    /** Dimensiune marker (vizual) */
    readonly radius: number = 1;
    /** Dimensiune colshape (logica de interacțiune) */
    readonly colshapeRadius: number = 3.0;
    /** Culoarea markerului */
    readonly color: RGBA = [255, 182, 193, 200];


    /** Rotația markerului */
    readonly rotation: number = 0;

    /** Tipul de text static */
    ...
    constructor(
        ...
        ids += 1000;
        this.id = ids;
        const positions = position instanceof Array ? position : [position];
        ...
        if (options) {
            ...
            if (options.colshapeRadius) this.colshapeRadius = options.colshapeRadius;
        ...
        // ✅ Creăm markerul mic + colshape mare
        positions.map((position, id) => {
            const colshape = mp.colshapes.newSphere(
                position.x,
                position.y,
                position.z + 1,
                this.colshapeRadius, // mare pentru interacțiune
                this.dimension
            );
            colshape.handleClass = this;

            if (this.type !== -1) {
                this.createMarker(position); // mic vizual
            }
        ...
        this.colshapes.map((colshape, index) => {
            if (mp.colshapes.exists(colshape)) {
                if (!mp.players.local) {
                    mp.players
                        .toArray()
                        .filter((target) => target.colshape == colshape)
                        .map((target) => {
                ...
                colshape.position = new mp.Vector3(
                    this.position[index].x,
                    this.position[index].y,
                    this.position[index].z + 1
                );
