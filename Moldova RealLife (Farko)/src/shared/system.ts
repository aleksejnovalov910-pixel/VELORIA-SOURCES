...
export class systemUtil {
    static toBinary(str: string) {
        let res: string[] = [];
        for (let i = 0; i < str.length; i++) {
            let charCode = str.charCodeAt(i);
            res.push(charCode.toString())
        }
        return res.join('_');
    }

    static fromBinary(encodedStr: string) {
        let data = encodedStr.split("_");
        let res: string = "";

        data.forEach(charCode => {
            res += String.fromCharCode(charCode as any);
        })
        return res;
    }
    ...
    static filterInput(text: string){
        ...
        //text = text.toString().replace(/[^A-Za-zА-Яа-яЁёЇїІі0-9 \-,.?/!%()$_*+=#№@;:<>{}\[\]]/g, '') ORIGINAL
        //text = text.toString().replace(/<([^>]*)>?/gm, '$1'); ORIGINAL
        //text = text.toString().replace(/"/g, '') ORIGINAL
        // text = text.toString().replace(/[^\x20-\x7E]/g, '')
        // text = text.toString().replace(/	/g, '')
        // text = text.toString().replace(/\\/g, '')
        //text = text.toString().replace(/'/g, '') ORIGINAL
        //text = text.toString().replace(/[^\wáéíóúüñÁÉÍÓÚÜÑ \-,.?/!%()$_*+=#№@;:<>{}\[\]]/g, '') 
        //text = text.toString().replace(/[^\wáéíóúüñÁÉÍÓÚÜÑäöüÄÖÜ가-힣 \-,.\?\/!%()$_*+=#№@;:<>{}\[\]]/g, '');
        text = text.toString().replace(/[^\wáéíóúüñÁÉÍÓÚÜÑäöüÄÖÜąęćźżłóĄĘĆŹŻŁÓŚńŃśČčĆćŽžĐđŠš가-힣 \-,.\?\/!%()$_*+=#№@;:<>{}\[\]]/g, '');
    ...
    */
    ...
    static createBlip(type: number, color: number, position: {x: number, y: number, z?: number}, name: string, dimension: number = 0, shortRange: boolean = true, scale: number = this.blipBaseScale) {
    ...
    static getRandomSpawn(): [Vector3Mp, number] {
        ...
        return randomArrayElement([
            [new mp.Vector3(-1052.41, -2763.90, 21.36), 335],
        ])
    }
    ...
    */
    ...
    /**
     * Шансы на взлом отмычкой наручников
     * 
     */
    static getLockHackingStatus(): boolean {
        ...
        const start = (items: TItem[]) => {
            const total = items.reduce(
                (accumulator: number, item: TItem) => accumulator + item.dropChance,
                0,
            );
    ...
    static getWeatherName(type: "EXTRASUNNY" | "CLEAR" | "CLOUDS" | "SMOG" | "FOGGY" | "OVERCAST" | "RAIN" | "THUNDER" | "CLEARING" | "SNOW" | "XMAS" | "HALLOWEEN" | "NEUTRAL"){
        switch (type) {
            ...
            case "CLEAR":
                ...
                return langStringDefault("weather.clear");
            case "CLOUDS":
                //return "Bewölkt";
                return langStringDefault("weather.clouds");
            case "SMOG":
                //return "Smog";
                return langStringDefault("weather.smog");
            case "FOGGY":
                //return "Nebel";
                return langStringDefault("weather.foggy");
            case "OVERCAST":
                //return "Bedeckt";
                return langStringDefault("weather.overcast");
            case "RAIN":
                //return "Regen";
                return langStringDefault("weather.rain");
            case "THUNDER":
                //return "Gewitter";
                return langStringDefault("weather.thunder");
            case "CLEARING":
                //return "Leichter Regen";
                return langStringDefault("weather.clearing");
            case "SNOW":
                //return "Schnee";
                return langStringDefault("weather.snow");
            case "XMAS":
                //return "Schnee";
                return langStringDefault("weather.xmas");
        }
        return "Klares Wetter";
        return langStringDefault("weather.clear");
    }
