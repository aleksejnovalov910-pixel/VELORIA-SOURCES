...
let markerVehicleRenderBound = false;
...
setInterval(() => {
    ...
    if (!markerVehicleRenderBound) {
        markerVehicleRenderBound = true;
        mp.events.add("render", markedVehicleRender);
    }

}, 500);


function markedVehicleRender() {
...
CustomEvent.registerServer(ENTER_VEHICLE_STEP_DESTROY_MARKER, () => {
    ...
    markedVehicle = null;
    markerVehicleRenderBound = false;
    mp.events.remove("render", markedVehicleRender);
});
