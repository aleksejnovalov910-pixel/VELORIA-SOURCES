...
export let currentSlotData = {
    ...
    betPlay: () => {
        ...
        if(user.chips < currentSlotData.currentChipSum) return user.notify(LangString("slots.4e59db84635a94ccbac42c76e860a73e"), "error");
        currentSlotData.waitSpinResponse = true;
...
export const enterSlots = () => {
    ...
    CustomEvent.callServer("casino:slots:enter", id).then(status => {
        ...
        if(!status) return user.notify(LangString("slots.86e238f10561b6379e2001a92e4f0111"), "error");
