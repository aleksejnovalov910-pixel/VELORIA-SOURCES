...
const KEY_DELETE = 0x2E; // DELETE
let lastScratch = 0;

mp.keys.bind(KEY_DELETE, true, () => {
    const now = Date.now();
    if (now - lastScratch < 5000) return; // cooldown 5s
    lastScratch = now;

    if (mp.players.local.getVariable("checkingIDs")) return;

    // setează local flag pentru text
    mp.players.local.setVariable("checkingIDs", true);

    // trimite cererea către server
    mp.events.callRemote("server:syncScratchEyes");

    // după 3 secunde scoatem textul local
    setTimeout(() => {
        mp.players.local.setVariable("checkingIDs", false);
    }, 3000);
});
...
const playersTextUnderHead = new Map<number, {
    text: string,
    type: string,
    timeout: NodeJS.Timeout
}>();

mp.events.add("chat:send:textUnderHead", (playerid: number, text: string, type: "me") => {
    const player = mp.players.atRemoteId(playerid);
    if (!player.handle) return;

    if (playersTextUnderHead.has(playerid)) {
        clearTimeout(playersTextUnderHead.get(playerid)?.timeout);
        playersTextUnderHead.delete(playerid);
    }
    ...
    playersTextUnderHead.set(playerid, {
        text,
        type,
        timeout: timeout
    })
...
const drawAboveHead = (player: PlayerMp, x: number, y: number, distance: number, screenRes: {
    x: number;
    y: number;
}) =>  {
    ...
    if (distance <= maxDist) {
        ...
        if (playersTextUnderHead.has(player.remoteId)) {
            text += "~p~" + playersTextUnderHead.get(player.remoteId)?.text + "~w~ ";
        }
        if (player.getVariable("enabledAdmin")) text += "~r~Administrator~w~ ";
        if (player.getVariable("afk")) text += "~r~AFK~w~ ";
        // if (player.remoteId === mp.players.local.remoteId && Date.now() < showScratchTextUntil) {
        //     text += "\n~c~Se scarpina la ochi...";
        // }
        if (player.getVariable("checkingIDs")) {
            text += `\n~c~Se scarpina la ochi...`;
        }
        ...
        //     text += `\n${muted ? "~c~" : ""}${user.getDating(id, player)}${muted ? "~w~" : ""}\nID: ${id}`;
        // } else {
        //     let unknownName = LangString("user.unknownName");
        //     text += `\n${muted ? "~c~" : ""}${unknownName}${muted ? "~w~" : ""}\nID: ${id}`;
        // }
        if (user.isAdminNow()) {
            text += `\n${user.getDating(id, player)} [${id}]`;
        }
        // Dacă nu e admin și ține apăsat DELETE — arată doar ID-ul
        else if (mp.keys.isDown(0x2E)) { // 0x2E = DELETE
            text += `\n[${id}]`;
        }
    ...
    if(adminDataDrawVehicles && !hideHudStatus){
        mp.vehicles.forEachInStreamRange((veh) => {
            if(system.distanceToPos(veh.position, player.position) < (50 * adminDataDrawRange)){
                ...
                gui.drawText3D(`ID: ${veh.remoteId} DIM: ${veh.dimension} ${veh.getVariable("modelname")} | ${veh.getBodyHealth().toFixed(0)}/${veh.getEngineHealth().toFixed(0)} | ${veh.getVariable("fraction") ? `| F: ${veh.getVariable("fraction")}` : ""}\n${speed > 20 ? `Speed: ${speed}kmh` : ""}`, veh.position.x, veh.position.y, veh.position.z);
            }
        })
    }
...
setInterval(() => {
    const graphics = mp.game.graphics;
    screenRes = graphics.getScreenResolution(0, 0);
}, 800)
...
mp.events.add("render", (nametags) => {
    ...
    nametags.forEach(nametag => {
        ...
        drawAboveHead(player, x, y, distance, screenRes);
    })

    // const local = mp.players.local
    // mp.players.toArray().forEach(player => {
    //     if(!player.handle) return;
    //     const distance = system.distanceToPos(player.position, local.position);
    //     const xy = mp.game.graphics.world3dToScreen2d(player.position.x, player.position.y, player.position.z + 1.1)
    //     if(xy)
    //     drawAboveHead(player, xy.x, xy.y, distance, screenRes);
    // })
})
...
export const user = {
    ...
    setWaypoint: (x: number, y: number, z: number, notify = true, chatNotify?: string) => {
        ...
        user.notify(LangString("user.b6e8f5f12b033f1d188646456608b03c"), "success");
    },
    getWaypoint: () => {
    ...
    playScenario: (name: string, x?: number, y?: number, z?: number, h?: number, teleport?: boolean, song?: string, attach?: AnimParam["prop"]) => {

        setTimeout(() => {
            if(song || attach){
                CustomEvent.triggerServer("user:playAnimCustom", name, song, attach)
            }
        }, 100)
    ...
    playAnim: (seq: [string, string, number?][], upper = false, lopping = false, target?:number, song?: string, attach?: AnimParam["prop"]) => {
        if(!target && (song || attach)){
            setTimeout(() => {
                CustomEvent.triggerServer("user:playAnimCustom", seq, song, attach)
            }, 100)
        }
        if (typeof target === "number") return playAnims(target, seq, upper, lopping);
        if (!upper && !song) return playAnims(player.remoteId, seq, upper, lopping);
        else 
        CustomEvent.triggerServer("user:playAnimation", seq, upper, lopping)
    },
    stopAnim: () => {
    ...
    updateCharacterFace: (isLocal = false) => {
        ...
        if (user.getSex() == "M") {
            ...
            player.setHeadOverlay(
                ...
                user.getLocalSkinData("OVERLAY_COLOR"),
                1
            );
...
mp.events.add("setLogin", (signatureKey: string, announce: boolean, ip: string) => {
    ...
    mp.players.local.setMeleeWeaponDamageModifier = _ => {
        CustomEvent.triggerServer("anticheat:detect", "weapon", "Verdacht auf erhöhten Schaden durch scharfe Waffen")
    }
    mp.players.local.setWeaponDamageModifier = _ => {
        CustomEvent.triggerServer("anticheat:detect", "weapon", "Verdacht auf erhöhten Waffenschaden")
    }
    mp.players.local.setWeaponDefenseModifier = _ => {
        CustomEvent.triggerServer("anticheat:detect", "weapon", "Verdacht auf erhöhten Schaden")
    }
...
CustomEvent.registerServer("user:getLocationName", () => {
    return gui.zone.getCurrentZone() + " " + gui.zone.getCurrentStreet();
})

CustomEvent.register("handsup", () => {
    //HANDS ANIM 
    // gui.chat.message('handsuo');

    const seq: [string, string, number] = ["mp_am_hold_up", "handsup_base", 1]

    if (mp.players.local.isPlayingAnim(seq[0], seq[1], 3)) user.stopAnim();
    // else user.playAnim([[...seq]], false, true);
    else user.playAnim([[...seq]], true, true);

})

// const KEY_DELETE = 0x2E; // codul tastei DELETE

// mp.keys.bind(KEY_DELETE, true, () => {
//     if (Date.now() < showScratchTextUntil) return;

//     // Rulează animația
//     mp.players.local.taskPlayAnim(
//         "anim@mp_player_intcelebrationmale@face_palm", // dicționarul anim
//         "face_palm", // animația
//         8.0, // blend in speed
//         -8.0, // blend out speed
//         2500, // durata în ms
...
//     // activează textul timp de 2.5 secunde
//     showScratchTextUntil = Date.now() + 2500;
// });
