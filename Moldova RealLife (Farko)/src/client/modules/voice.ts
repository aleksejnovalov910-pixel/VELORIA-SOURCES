...
mp.events.add('saveVoiceSettings', (params: string) => {
    ...
    user.notify("Voice settings saved", "success");
    setTimeout(() => {
        voiceConfig()
    }, 100)
})
...
CustomEvent.register('vreload', () => {
    ...
    user.notify('Voice chat reloaded', 'success');
})
...
export function restartVoice() {
    if (block) return mp.game.ui.notifications.show('~r~You cant reload voice chat so often');
    block = true;
    setTimeout(() => {
        block = false;
    }, 10000)
    mp.voiceChat.cleanupAndReload(true, true, true);
    mp.game.ui.notifications.show('~g~Voice chat reloaded');
}
...
let prevRemoveArray: number[] = [];
let prevAddArray: number[] = [];
...
function voiceControllerTick() {
    ...
    if (arraysEqual(playersToRemove, prevRemoveArray) && arraysEqual(playersToAdd, prevAddArray)) return;

    CustomEvent.triggerServer("worldVoiceChangeScope", playersToRemove, playersToAdd);

    prevRemoveArray = [...playersToRemove];
    prevAddArray = [...playersToAdd];
}
...
const enableMicrophone = async () => {
    ...
    if (user.dead)
        return;
    if (mp.players.local.getVariable('muted:voice')) return user.notify(`You cant use voice chat until ${system.timeStampString(mp.players.local.getVariable('muted:voice'))}.`, 'error');
...
const enableMicrophoneRadio = () => {
    ...
    if (mp.players.local.getVariable('muted:voice')) return user.notify(`You cant use radio until ${system.timeStampString(mp.players.local.getVariable('muted:voice'))}.`, 'error');
