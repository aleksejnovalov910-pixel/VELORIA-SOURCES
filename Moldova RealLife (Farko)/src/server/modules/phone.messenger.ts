...
class PhoneMessenger {
    ...
    public async sendMessage(player: PlayerMp, myPhoneId: number, myNumber: number, targetNumber: number, text: string, withGps?: boolean): Promise<void> {
        ...
        if (!targetPhone)
            return user.notifyPhone(user.LangString("phone.messenger.a5d7ebb58e772cd341ade11410634541"), user.LangString("phone.messenger.65b26c347d28798fd188e32f164cf2ba"), user.LangString("phone.messenger.1285a570f1778401d2d882b11ce0f9ea"))
        ...
        if (targetPlayer.phoneReadMessage != myNumber)
            return targetPlayer.user.notifyPhone(targetPlayer.user.LangString("phone.messenger.8a2161ad4fc21b20c49678eb0bcc4158"), `${(await phone.getContactName(targetPhone.id, myNumber)) || langStringDefault("phone.messenger.ca1dab43059480ec166e73c43c819ea3")} (${myNumber})`, `${text}`)
    ...
    public async sendGpsMessage(player: PlayerMp, myPhoneId: number, myNumber: number, targetNumber: number): Promise<void> {
        return this.sendMessage(player, myPhoneId, myNumber, targetNumber, player.user.LangString("phone.messenger.19ae99b7a47c6a15f88d92a786d2f2a2"), true)
    }
