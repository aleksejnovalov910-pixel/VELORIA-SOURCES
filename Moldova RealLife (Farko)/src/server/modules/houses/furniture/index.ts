...
class Furniture {
    ...
    private placementHandler = (player: PlayerMp, id: number, toggle: boolean) => {
        ...
        if (!this.isInteriorForFurniture(player.user.houseEntity.interrior))
            return player.notify(player.user.LangString("index.a9834ed93c38e83aa733c856c453abc5"));
    ...
    private buyFurnitureHandler = (player: PlayerMp, cfgId: number) => {
        ...
        if (!player.user.houseEntity)
            return player.notify(player.user.LangString("index.e11356cbb2db74bf63f28aaa2acf5164"), "error");

        if (!this.isInteriorForFurniture(player.user.houseEntity.interrior))
            return player.notify(player.user.LangString("index.12df6ba974791aa511a074ddc984f99f"), "error");
        ...
        if (player.user.money < cfg.cost)
            return player.notify(player.user.LangString("index.37a3dbfb32d550068a2dec8e9a53bb17"), "error");

        player.user.removeMoney(cfg.cost, false, player.user.LangString("index.cd14a301e19e249a9a8bf675ae558734", cfgId));
        ...
        player.user.log(langStringDefault("index.419f290c549b14c711a5349c5cdbb28a"), langStringDefault("index.4bf18058349a8eebd31dbf0d128a6968", cfg.id, cfg.cost));
        player.notify(player.user.LangString("index.b4f0d45d0229a9e6b59d557f9a042c89", cfg.name), "success");
    }
    ...
    private sellHandler = (player: PlayerMp, id: number) => {
        ...
        if (item.pos)
            return player.notify(player.user.LangString("index.168c129a49244bf844fe4065b0aead35"));
        ...
        player.user.addMoney(Math.floor(cfg.cost * 0.2), true, player.user.LangString("index.f7815f91a5e68a9706e48b85c1efdd0d", cfgId));
    }
