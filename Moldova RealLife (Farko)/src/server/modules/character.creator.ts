...
CustomEvent.registerClient('server:user:personage:done', (player, datastring: string, name: string, family: string, age: number, dresss?: string, promo?:string) => {
    ...
    if(!user.name && name && family){
        ...
        CustomEvent.triggerClient(player, "enableStartCutscene");
    } else {
        user.returnToOldPos()
    }
})

CustomEvent.registerClient('server:user:personage:doneCutscene', (player) => {
    const spawn = system.getRandomSpawn();
    player.position = spawn[0];
    player.heading = spawn[1];
