...
class Session implements SessionInfo {
    ...
    public joinPlayerToSession(player: PlayerMp, password: string) {
        ...
        if (this.players.length == this.Type.MaxPlayers) {
            player.notify(player.user.LangString("jobSessions.782516a9cebd82755ef4377062954b64"), "error");
            return;
        }

        if (this.IsProtected && password.toUpperCase() !== this.password) {
            player.notify(player.user.LangString("jobSessions.e0233ac7dbcd19ba29898b4e3bfd4aee"), "error");
            return;
        }
...
CustomEvent.registerCef("sessions::create", (player, withPassword: boolean) => {
    ...
    if (session) {
        player.notify(player.user.LangString("jobSessions.0999e3cf822366b56a82e998880e1cb9"), "error");
        return;
    }
