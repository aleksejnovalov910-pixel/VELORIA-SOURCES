...
export const gangfight = {
    ...
    create: (index: number) => {
        ...
        mp.players.toArray().filter(q =>
            ...
            target.notify(target.user.LangString("gangfight.ed2c832bc01c1836ca70e4ff2a4bb230", cfg.name), 'success')
        );
        let timer = cfg.timer * 60;
        const text = () => {
            return langStringDefault("gangfight.4160fb43db4487af8883eccbda27e226", system.secondsToString(timer))
        }
        ...
        let int = setInterval(() => {
            ...
            if (!timer) {
                ...
                const colshape = colshapes.new(new mp.Vector3(cfg.x, cfg.y, cfg.z), cfg.name, player => {
                    ...
                    if(!user) return;
                    if(!cfg.factions.includes(user.fraction)) return player.notify(player.user.LangString("gangfight.ab1611ed5ff500c693cd65fd4aa401e5"), 'error');
                ...
                clearInterval(int);
                if (mp.labels.exists(label)) label.text = langStringDefault("gangfight.a415a5260b0006ca69d6f22c2340e619");
                ...
                setTimeout(() => {
                    ...
                    const text = () => {
                        return langStringDefault("gangfight.b63c4b055dc76a36c0bba3c3935198cf", system.secondsToString(timer))
                    }
