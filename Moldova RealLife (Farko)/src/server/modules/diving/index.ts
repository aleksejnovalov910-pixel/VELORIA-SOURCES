...
import {gui} from "../gui";
import { DEFAULT_CHESTS } from "../../../shared/diving/chests.config";
...
async function giveMap(player: PlayerMp) {
    // Alege o locație random din lista de locații disponibile
    const nextLocation = DEFAULT_CHESTS[Math.floor(Math.random() * DEFAULT_CHESTS.length)];

    // Trimite evenimentul către client ca să creeze noul checkpoint
    CustomEvent.triggerClient(player, "diving:nextCheckpoint", nextLocation);

    // player.user.log("diving", langStringDefault("index.eca3cfe2a3b1927a52d5059f85ea7627", DEFAULT_CHESTS));
...
//     player.user.log("diving", langStringDefault("index.eca3cfe2a3b1927a52d5059f85ea7627", randSlice));
...
CustomEvent.registerClient("diving:dive", (player) => {
    if (!player.user.haveItem(DIVING_COSTUME_ITEM_ID))
        return player.notify(player.user.LangString("index.78a12505c8d94ba69cc121c66c6b7ac6"));
...
CustomEvent.registerClient("diving:reward", (player, isMapMission: boolean) => {
    ...
    // Parcurge toate itemele din lista REWARDS și le oferă câte 1 fiecăruia
    for (const rewardItem of REWARDS) {
        player.user.giveItem(rewardItem, true);
        player.user.log("diving", langStringDefault(
            "index.0e16a43581c0fff9d719b621a651c24c",
            rewardItem
        ));
    }

    // După ce primește toate itemele, îi dă și următoarea hartă
    giveMap(player);
});

CustomEvent.registerClient("diving:deleteMapItem", (player, item_id: number) => {
    inventory.deleteItemsById(player, item_id, 1);
    player.notify(player.user.LangString("index.46f3f000fcccd18780b62a472521ca79"), "success");
});
