...
export class UserBattlePassManager {
    ...
    public giveEveryDayExp() {
        ...
        this._user.player.notify(langStringDefault("userBattlePassManager.ccf555ec1b59997a486843a0d0199c48", this._season.everyDayExp.exp),
            "info");
    }
    ...
    buyBattlePass() {
        if (this.battlePassEntity)
            return this._user.notify(langStringDefault("userBattlePassManager.0e3e24623104adeadd3ac15bcc239fb6"));
        ...
        if (this._user.donate_money < cost)
            return this._user.notify(langStringDefault("userBattlePassManager.cfe798837a7d3500908ad9322088505c"), "error");

        this._user.donate_money = this._user.donate_money - cost;
        this._user.notify(langStringDefault("userBattlePassManager.6502fddc30f7c442fcdfa1b953eb4d66"), "success");
        this.createBattlePassEntity();
        this.loadGlobalTask();
        this.loadBasicTasks();
        this._user.log("battlePass", langStringDefault("userBattlePassManager.bcaae1f36c2c3796e21153c6aa4bad1a"));
    ...
    public isSpam() {
        if (system.timestampMS - this.antiSpam < 2000) {
            this._user.player.notify(langStringDefault("userBattlePassManager.eda33e78b9d541f8c73dc78238e83874"), "error");
            return true;
        }else{
    ...
    updateGlobalTask(goalsCount: number, isCompleted: boolean = false, expReward: number): void {
        if (isCompleted) {
            ...
            this.battlePassEntity.save();
            this._user.log("battlePass", langStringDefault("userBattlePassManager.b6745e24c2d5c1de060f7956df593549", expReward));
        } else {
            const globalTask = {...this.battlePassEntity.globalTask};
    ...
    updateBasicTask(id: number, goalsCount: number): void {
        ...
        if (goalsCount === cfg.goal) {
            ...
            this._user.log("battlePass", langStringDefault("userBattlePassManager.cf36b9e57bf4658c5d2e0b34b040eddf", cfg.expReward));
        }
    ...
    public giveReward(level: number): void {
        if (this.isRewardReceived(level))
            return this._user.notify(langStringDefault("userBattlePassManager.b4b80ecc1de6919ade22b4c6b360a7d1"));
    ...
    public buyLevels(levels: number): void {
        ...
        if (this._user.donate_money < cost)
            return this._user.notify(langStringDefault("userBattlePassManager.c5d5da39f95ad97701446c18a85a3bb5"));

        this._user.donate_money = this._user.donate_money - cost;

        this.addExp(this._season.levelExp * levels);
        this._user.player.notify(langStringDefault("userBattlePassManager.4ec713bc947516e8c6918c547c16fd59", this.getLevel()), "success");
        this._user.log("battlePass", langStringDefault("userBattlePassManager.0fe04ce7424b4d296d37a04a1a3cefcd", levels));
    }
