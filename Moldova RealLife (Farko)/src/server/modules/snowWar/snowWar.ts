...
export class SnowWar {
    ...
    private registerPlayer(player: PlayerMp): void {
        if (this.battleStarted) return player.user.notify(player.user.LangString("snowWar.13e305b642c953d062e0ca4fd3760573"), "error");
        if (this.players.length >= this.maxPlayers)
            return player.notify(player.user.LangString("snowWar.d406da29f94c0d1ccc4351f649b926da"), "error");
    ...
    private registerShotHandle(player: PlayerMp, targetId: number) {
        mp.gui.chat.push(`player.weapon:${player.weapon}requested weapon:${SNOW_WAR_WEAPON_HASH}fromPlayer:${player}targetID:${targetId}`);
