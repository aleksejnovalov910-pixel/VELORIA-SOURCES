...
CustomEvent.registerClient('interaction:huntingPed', (player: PlayerMp, pedId: number) => {
    ...
    const check = (checkItem: boolean = true) => {
        ...
        if(checkItem && !user.haveItem(HUNTING_SKINNING_WEAPON)) {
            player.notify(player.user.LangString("hunting.5726fe431c61007f278995545b5225ed", getBaseItemNameById(HUNTING_SKINNING_WEAPON)), 'error');
            return false;
        }
    ...
    if(animalHunting){
        interaction.add(langStringDefault("hunting.856289b140aef08d9364a82a8ccad74a"), '', 'swords',() => {
            if(!check()) return;

            user.playAnimationWithResult(["amb@medic@standing@kneel@base" ,"base"], HUNTION_SKINNING_SECONDS, user.LangString("hunting.9ffe760f5fd644f919b5eec7e2c46846")).then((status) => {
                if(!check()) return;
                if(!status) return;
