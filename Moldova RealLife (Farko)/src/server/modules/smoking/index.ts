    ...
    bongAction() {
        this.smokingActions.set(BONG_ITEM_ID, (player: PlayerMp, item: ItemEntity) => {
            if (system.timestamp - player.user.lastSmoke < 5)
                return player.notify('Este prea rapid', 'error');

            const cbd = inventory.getInventory(OWNER_TYPES.PLAYER, player.user.id)
                .find(el => el.item_id === MARIHUANA_ITEM_ID);

            if (!cbd)
                return player.notify('Nu ai plic cu marihuana pentru bong', 'error');

            inventory.deleteItemsById(player, MARIHUANA_ITEM_ID, 1);
