...
export class Animal extends BaseActivity {
    ...
    private async suggestFeed(player: PlayerMp): Promise<ItemEntity> {
        return new Promise(async (resolve, reject) => {
            const m = menu.new(player, "", player.user.LangString("animal.05abfc6ac50c862d16a699b866d98deb"));
            m.sprite = "farm"
            m.onclose = () => {
                resolve(null)
            }
            player.user.inventory
                ...
                .map(inventoryItem => {
                    m.newItem({
                        name: langStringDefault("animal.18aa69220f10f97c93d59a2762983b86", inventoryShared.get(inventoryItem.item_id).name, inventoryItem.count),
                        type: "select",
    ...
    public async onPlayerEnterWorkPoint(player: PlayerMp, id: number): Promise<void> {
        ...
        if (!animal || animal.state == FarmAnimalState.Idle) 
            return player.notify(player.user.LangString("animal.299b0163104d439f5b38695b73a020dd"), "error")
        ...
        switch (animal.state) {
            case FarmAnimalState.Feed:
                ...
                const gameCompleted = await player.user.waitTimer(1, 5, player.user.LangString("animal.e8a5296c9993dc13fc645d288e1ec13f", inventoryShared.get(feed.item_id).name.toLowerCase()), ["anim@heists@money_grab@duffel", "loop", true])
