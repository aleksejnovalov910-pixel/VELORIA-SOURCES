...
export class Landing implements IFieldStage {
    ...
    private async suggestSelectSupply(player: PlayerMp): Promise<ItemEntity> {
        return new Promise(async (resolve, reject) => {
            const m = menu.new(player, "", player.user.LangString("field.4af12d943a3c26c4b03e7be748b02981"));
            m.sprite = "farm"
            m.onclose = () => {
                resolve(null)
            }
            player.user.inventory
                ...
                .map(inventoryItem => {
                    m.newItem({
                        name: langStringDefault("field.07f03debd83dfe763de32b4c38d4d97c", inventoryShared.get(inventoryItem.item_id).name, inventoryItem.count),
                        type: "select",
    ...
    public async startGameWithResult(player: PlayerMp, pointIdx: number): Promise<boolean> {
        ...
        const gameCompleted = await player.user.waitTimer(1, gameTime, player.user.LangString("field.a24c74508122468ab23a08f7c57d21b6", inventoryShared.get(inventoryItem.item_id).name.toLowerCase()), ["anim@heists@money_grab@duffel", "loop", true])
