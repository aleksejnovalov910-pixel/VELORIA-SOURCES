...
export class Landing implements IGreenhouseStage {
    ...
    private async suggestSelectSupply(player: PlayerMp): Promise<ItemEntity> {
        return new Promise(async (resolve, reject) => {
            const m = menu.new(player, "", player.user.LangString("greenhouse.1269859f71620edf5107574105c7a81e"));
            m.sprite = "farm"
            m.onclose = () => {
                resolve(null)
            }
            player.user.inventory
                ...
                .map(s => s.inventoryItemId).includes(i.item_id)).map(inventoryItem => {
                    m.newItem({
                        name: langStringDefault("greenhouse.5aef79622abcd0629f2714f8efc7cd1c", inventoryShared.get(inventoryItem.item_id).name, inventoryItem.count),
                        type: "select",
    ...
    public async startGameWithResult(player: PlayerMp, pointIdx: number): Promise<boolean> {
        ...
        const gameCompleted = await player.user.waitTimer(1, gameTime, player.user.LangString("greenhouse.797132b322d514d1dcc2edfdc75eb73d", inventoryShared.get(inventoryItem.item_id).name.toLowerCase()), ["anim@heists@money_grab@duffel", "loop", true])
