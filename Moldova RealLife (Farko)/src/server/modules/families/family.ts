...
export class Family {
  ...
  constructor(entity: FamilyEntity) {
    ...
    this.familyChat = dialogSystem.createDialogFamily(
      langStringDefault("family.1dc79cf5c35fceac4be4fff46734295e", this.name),
      this.id,
      20
    );
  }
  ...
  static load() {
    ...
    return new Promise((resolve) => {
      FamilyEntity.find().then((f) => {
        ...
        system.debug.success(langStringDefault("family.27dda7d8b4ec6c9a496e2a92f4891c8c", f.length));
        resolve(null);
      });
    });
  }
  ...
  addPoints(count: number, withSeason = true) {
    ...
    system.saveLogFamily("addPoints", count, langStringDefault("family.4cde7eb77266fb555a2393fa323ed0d8", withSeason), this.entity);
  }
  ...
  clearSeasonPoints(byAdmin = false) {
    ...
    familyChat.messages.push({
      ...
      text: `${
        byAdmin
          ? langStringDefault("family.a2c6b4f47af634245b8223f957b756f7")
          : langStringDefault("family.e85cfa30e19d0be9dbe5423c70c0f564")
      }`,
    });
  }
  ...
  removeMoney(val: number, player?: PlayerMp, reason?: string) {
    ...
    system.saveLogFamily(
      "removeMoney",
      val,
      reason ? reason : langStringDefault("family.985e7d6257dedc4f266ee1eeec47a5a1"),
      this.entity,
      player
    );
    this.money -= val;
  ...
  addMoney(val: number, player?: PlayerMp, reason?: string) {
    ...
    system.saveLogFamily(
      "addMoney",
      val,
      reason ? reason : langStringDefault("family.0fea5f25d1c90242ae1504356c961899"),
      this.entity,
      player
    );
    this.money += val;
  ...
  removeDonateMoney(val: number, player?: PlayerMp, reason?: string) {
    ...
    system.saveLogFamily(
      "removeDonate",
      val,
      reason ? reason : langStringDefault("family.aed2ce0257e9c458447ae317576716b4"),
      this.entity,
      player
    );
    this.donate -= val;
  ...
  addDonateMoney(val: number, player?: PlayerMp, reason?: string) {
    ...
    system.saveLogFamily(
      "addDonate",
      val,
      reason ? reason : langStringDefault("family.2245ac042a9f4afb98a9cfe6a734c0c0"),
      this.entity,
      player
    );
    this.donate += val;
    ...
    if ((needContracts[id] * 100) / contract.needScore >= 100) {
      contract.win.map((win) => {
        if (win.type == FamilyContractWinTypes.MONEY)
          this.addMoney(win.amount, null, langStringDefault("family.05698ecf5ce30f8f2d551a6fa667ed5c"));
        if (win.type == FamilyContractWinTypes.COINS)
          this.addDonateMoney(
            win.amount,
            null,
            langStringDefault("family.9cc145df2d93a8a280c5092db8456cb5")
          );
    ...
    return new Promise<number>(async (resolve) => {
      ...
      if (player.user.level <= 1)
        return player.notify(
          player.user.LangString("family.dcc9160dd6aded33f64de3692e43081d")
        );
      if (
        await player.user.tryPayment(
          ...
          player.user.LangString("family.7ebe8a7ced5915eac55656b8a82ac8d3"),
          player.user.LangString("family.b606a0f95e6fbb5ed73da22d4edd0203", player.user.family.id)
        )
      ) {
        player.notify(
          player.user.LangString("family.793e7f5c29bf5e4f0f5bdf5f466939fb", type ? player.user.LangString("family.c4cad64240c411384e9bc249f0083b97") : player.user.LangString("family.2cf1be0743eaaddde028ab9c9d66af65"), type ? `${count}` : player.user.LangString("family.18e63335d61dd790934a520ad6eea757", count)),
          "success"
        );
        if (type) this.addMoney(count, player, langStringDefault("family.e0203f8b0b44891f9e0506c42d1458c2"));
        else this.addDonateMoney(count, player, langStringDefault("family.bd90dd99cbefdeca11ff159fecae4d11"));
  ...
  playerKickPlayer(player: PlayerMp, id: number) {
    return new Promise<boolean>(async (resolve) => {
      ...
      if (!player.user.family.isCan(player.user.familyRank, "kick")) {
        player.notify(player.user.LangString("family.90753f5c58ebc9f06207342c1171bb42"), "error");
        return resolve(false);
      }
      ...
      if (user) {
        player.notify(player.user.LangString("family.207b4d2cedb34a207c823ec235604957", user.user.name), "info");
        if (user.user.family != player.user.family) {
          player.notify(player.user.LangString("family.96ec37c603dfe63bcff09cb0d356b31b"), "error");
          return resolve(false);
        }
        user.user.family = null;
        user.user.notify(user.user.LangString("family.babfb97add54bf6c2d76566432e2aa24", player.user.name), "info");
      } else {
        let member = await this.getMemberByID(id);
        if (!member) {
          player.notify(player.user.LangString("family.20f7fe615d424d6c80ae5c3a245d53fc"), "error");
          return resolve(false);
        }
        member.family = 0;
        member.familyRank = 0;
  ...
  async playerLeaveFamily(player: PlayerMp) {
    ...
    if (player.user.isFamilyLeader) {
      ...
      if (family.cars.length || family.house)
        return player.notify(
          player.user.LangString("family.f8145d4317c831fefb2daa292159ebae")
        );
      if ((await family.getMembersCount()) > 1)
        return player.notify(
          player.user.LangString("family.af4af61309bbd823d3ec87bae33c60ab")
        );
      player.user.addMoney(
        family.money,
        true,
        player.user.LangString("family.f5b6a8991d2b6a3885de94e31c850e66")
      );
      player.notify(player.user.LangString("family.02d20d5b858eddea6a676f1ebdd7e723", family.name));
    ...
    player.user.family = null;
    player.notify(player.user.LangString("family.e8c1cc7bd5fc3bb3788e0ac45de3e372"));
    return;
  }
  ...
  playerSetRankPlayer(player: PlayerMp, id: number, rank: number) {
    return new Promise<boolean>(async (resolve) => {
      ...
      if (!player.user.family.isCan(player.user.familyRank, "setRank")) {
        player.notify(
          player.user.LangString("family.53c78bc6f0a766d69eb0f8515ae4fa15"),
          "error"
        );
        return resolve(false);
      }
      ...
      if (user) {
        player.notify(
          player.user.LangString("family.d227454ff04230ac4e1a2517fa4a7071", user.user.name, newRank.name),
          "info"
        );
        if (user.user.family != player.user.family) {
          player.notify(player.user.LangString("family.e421a8c1379dc126d2e7886a2440928f"), "error");
          return resolve(false);
        }
        user.user.familyRank = rank;
        user.user.notify(
          user.user.LangString("family.dbf2c9a9db0d0d4252f1212ba053b691", player.user.name, newRank.name),
          "info"
        );
      } else {
        let member = await this.getMemberByID(id);
        if (!member) {
          player.notify(player.user.LangString("family.9099c02f6e1610c3e67d9311f59d7e2b"), "error");
          return resolve(false);
        }
  ...
  tryLevelUp(player: PlayerMp, type: number): Promise<boolean> {
    return new Promise<boolean>(async (resolve) => {
      ...
      if (!player.user.family.isCan(player.user.familyRank, "level_up")) {
        player.notify(player.user.LangString("family.4342bcc4214724ccf41b7fa26265d4d1"), "error");
        return resolve(false);
      }
      const requires = LevelInfo[this.level];
      if (!type) {
        if (this.level >= LevelInfo.length) {
          player.notify(player.user.LangString("family.d315e1350b26d6372903946fa5c9ec7d"), "error");
          return resolve(false);
        }
        ...
        if (
          ...
          player.notify(
            player.user.LangString("family.1332ea19169cac50380909d84c3da6ae"),
            "error"
          );
          return resolve(false);
        }
        ...
        if (
          !player.user.tryRemoveDonateMoney(
            requires.coin,
            true,
            player.user.LangString("family.52a64d3005b41813eac4f17c4d37ea90")
          )
        )
          return resolve(false);
      ...
      player.notify(player.user.LangString("family.549d136e3211a20ccdbe93e7affdac95"), "success");
      this.level += 1;
      return resolve(true);
    });
  }
