...
export class CargoBattleFamilyQuest {
    ...
    async startReady(isAdminStart = false) {
        return new Promise((resolve, reject) => {
            ...
            if(families.length < this.minimumStartFamilies) {
                this.destroy()
                return reject('NOT ENOUGH FAMILYMEMBERS');
            }
    ...
    start() {
        ...
        if(!allObjects.length) {
            this.destroy()
            return system.debug.error('no free space- contact Igor.')
        }
        CargoBattleFamilyQuest.clearRestartTimeout()
    ...
    private enterZone(player: PlayerMp) {
        ...
        if(!player.user.isAdminNow() && player.user && player.user.family && player.user.health > 0) {
            if(!this.familyInGame.includes(player.user.family)) return player.notify(player.user.LangString("cargoBattle.youCant"))
    ...
    private setWinner() {
        if(this.attackFamily && this.attackFamily.id) {
            ...
            this.functionToAllFamilies(player => player.user.notify(`Familia ${this.attackFamily.name} a castigat lupta pentru marfa`))
            let allScores = 0
            this.players.filter(p => mp.players.exists(p.player) && p.player.user && p.player.user.family === this.attackFamily).map(winner => {
                ...
                if(!win) return;
                winner.player.notify(`Ai primit ${win} puncte de familie pentru participarea la lupta de marfa`)
