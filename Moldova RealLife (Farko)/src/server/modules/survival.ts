...
CustomEvent.registerClient('illdata:heal', (player, targetID: number, ill: IllId, cost: number) => {
    ...
    target.user.tryPayment(cost, 'all', () => checkPrice() && checkPlayers(), langStringDefault("survival.e493dcce29b0ef0a2308432f08729fdd", cfg.name), 'EMS').then(status => {
        if(!status) return;
        target.user.setIll(ill, 0);
        player.notify(langStringDefault("survival.b8f8fcca3c1fd6abb495afb55025cb17"), 'success');
        target.notify(langStringDefault("survival.10a3f5488f06a0d29fe36b1302c4023f", cfg.name), 'success');
        
        CustomEvent.triggerClient(player, 'markDeath:destroy')// Entferne den Marker eines toten Spielers, wenn es einen gibt
        
        const chest = MoneyChestClass.getByFraction(FACTION_ID.EMS);
        if(chest) chest.addMoney(player, cost * 0.3, false)
        user.addMoney(cost * 0.7, true, langStringDefault("survival.b3a0952e4e6d27dd2cfaf2e49ef1aa00", cfg.name));
    })
...
CustomEvent.registerClient('survival:death', (player, x: number, y: number, z: number) => {
    ...
    setTimeout(() => {
        ...
        user.heal_time = user.isAdminNow() ? 1 : 30;
        user.health = 100;
...
CustomEvent.registerClient('heal:payauto', (player) => {
    ...
    if (!user) return false;
    if (user.health >= 100) {
        user.notify("Esti sanatos, nu ai nevoie de tratament.", "error");
        return false;
    }
    // // ✅ Verifică dacă există medici online
    // const medicsOnline = mp.players.toArray().filter(p => p.user && p.user.fraction === 16).length;
    // if (medicsOnline > 0) {
    //     user.notify("Exista personal medical online, contacteaza un medic!", "error");
    //     return false;
    // }
    const sum = user.haveActiveLicense('med') ? QUICK_HEAL_COST.AUTO_LICENSE : QUICK_HEAL_COST.AUTO;
    if (user.money < sum) return false;
    user.removeMoney(sum, true, langStringDefault("survival.211172dcabfae38e2ea64eccf4d73239"));
    const chest = MoneyChestClass.getByFraction(16);
    if(chest) chest.addMoney(null, sum * 0.2, false);
    return true;
})
CustomEvent.registerClient('heal:tryrun', (player, x: number, y: number, z: number, h: number) => {
    const user = player.user;
    if (!user) return;
    user.anticheatProtects(['heal', 'teleport']);
    player.dimension = 0;
...
colshapes.new(SURGEON_POS, langStringDefault("survival.8ec81c1ce1f3ce672d3a511cc6c5e891"), (player) => {   
    const user = player.user;
    if(!user) return;
    user.tryPayment(SURGEON_COST, 'all', () => true, langStringDefault("survival.04a17cce6a72c4406f2751c6a3d4b5c7"), 'EMS').then(status => {
        if(!status) return;
