...
export class DeathMath {
    ...
    async exit(){
        ...
        this.spectators.map(item => {
            item.player.outputChatBox(langStringDefault("deadmatch.2adf1df3e452a1097c56e3ba4f3e55b1", winner == 1 ? this.team1_name : this.team2_name));
            CustomEvent.triggerClient(item.player, 'deathmath:stop')
            menu.accept(item.player, langStringDefault("deadmatch.cb746e664139c722086ee7a6db2c5198"), 'small', 60000).then(status => {
        ...
        if(this.exitTimeout) {
            this.totalpool.map(item => {
                ...
                user.drawTimer(user.LangString("deadmatch.9f8144a6cc5e77d926f7dd4216017ed6"), this.exitTimeout);
            });
