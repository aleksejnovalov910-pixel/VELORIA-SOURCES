...
setTimeout(() => {
    ...
    colshapes.new(new mp.Vector3(DRIFT_MAP_ENTER.x, DRIFT_MAP_ENTER.y, DRIFT_MAP_ENTER.z), 'Exit', player => {
...
CustomEvent.registerClient('drift:score', (player, score: number) => {
    ...
    if(!todayBestDriftScore.has(user.id) || todayBestDriftScore.get(user.id).score < score) {
        if(todayBestDriftScore.has(user.id) && todayBestDriftScore.get(user.id).score > 5000) player.outputChatBox(`Ai stabilit un nou record astazi. Recordul anterior a fost ${todayBestDriftScore.get(user.id).score}, iar noul record este ${score}`)
    ...
    if(user.entity.drift_best < score) {
        if(user.entity.drift_best > 5000) player.outputChatBox(`Ai stabilit un nou record. Recordul anterior a fost ${user.entity.drift_best}, iar noul record este ${score}`)
