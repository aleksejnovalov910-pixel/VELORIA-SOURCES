    ...
    nodes: [
        {
            id: 0,
            npcReplies: [
                { text: langStringDefault("exchange.f94d3f9d1b6a910962b19f476d486159") }
            ],
            answers: [
                {
                    text: langStringDefault("exchange.cd2ddfe1c90f494064239aec838d11be"),
                    toNode: 2
                }
            ]
        },

        {
            id: 1,
            npcReplies: [
                { text: langStringDefault("exchange.17b9c627cb79e329512d5826e9821b49") },
                { text: langStringDefault("exchange.29196cf995a8d535a8940c2420fdd523") }
            ],
            answers: [
                { text: langStringDefault("exchange.da3b048a8e86b67d9afab85c052fbda2"), isExit: true }
            ]
        },

        {
            id: 2,
            npcReplies: [
                { text: langStringDefault("exchange.b7a4115c3b344949050bbfa253b4688e") }
            ],
            answers: [
                { text: langStringDefault("exchange.c5aebb2a1bb423e202b5f26654f42114"), isExit: true, onReply: new FuncAnswer(openHalloweenMenu)  }
            ]
        }
    ]
})
...
async function buyHalloweenItem(player: PlayerMp, itemIdx: number) {
    ...
    const checkCandies = () => {
        if (player.user.candyCount < item.price) {
            player.notify(player.user.LangString("exchange.c44252a04e22faeb60dba52f6a073430"), "error")
            return false;
        }
