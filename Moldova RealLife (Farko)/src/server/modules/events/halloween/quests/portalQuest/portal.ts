...
const enterPortalColshape = new colshapeHandle(HALLOWEEN_PORTAL_POSITION, player => player?.user?.LangString("portal.c28d0e2822a4dd80afaf88a728feb51a") ?? langStringDefault("portal.c28d0e2822a4dd80afaf88a728feb51a"), handlePlayerEnterPortal,
    {
        type: -1,
        radius: 5
    }
...
function handlePlayerEnterPortal(player: PlayerMp, index: number) {
    ...
    if (!player.user.advancedQuests.isQuestActive(HALLOWEEN_PORTALS_QUEST_ID)) {
        player.notify(player.user.LangString("portal.77dbf93d28ba41b178e3bad649846ad1"), "error");
        return;
    }

    if (inventory.getItemsCountByType(player, ITEM_TYPE.WEAPON) < 1) {
        player.notify(player.user.LangString("portal.dab60c75465ef48e33e6a6ea4b570bfc"), "warning");
    }

    player.notify(player.user.LangString("portal.2f93f2c5c46f3420772e7be29418f0ca"));
}
