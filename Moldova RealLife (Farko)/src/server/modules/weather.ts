...
export const weather = {
    ...
    adminMenu: (player: PlayerMp) => {
        ...
        if (!user.hasPermission("admin:weather:set")) return user.notify(user.LangString("weather.adminmenu.noperm"), "error");
        let m = new MenuClass(player, player.user.LangString("weather.adminmenu.title"), player.user.LangString("weather.adminmenu.subtitle"));
        // m.sprite = "admin";
        m.newItem({
            name: player.user.LangString("weather.adminmenu.item1"),
            more: player.user.LangString(weather.freezeWeather ? "weather.adminmenu.item1.more1" : "weather.adminmenu.item1.more2"),
            onpress: () => {
                weather.freezeWeather = !weather.freezeWeather;
                if(weather.freezeWeather){
                    user.notify(user.LangString("weather.adminmenu.item1.notify1"), "success")
                } else {
                    user.notify(user.LangString("weather.adminmenu.item1.notify2"), "success")
                }
            }
        })
        m.newItem({
            name: user.LangString("weather.adminmenu.item2"),
            type: "list",
            ...
            onpress: (itm) => {
                ...
                user.notify(user.LangString("weather.adminmenu.item2.notify"), "success");
            }
        })
        // m.newItem({
        //     name: "Часы",
        ...
        //         weather.sync();
        //         user.notify('Время Eingerichtetо', "success");
        //     }
        // })
        // m.newItem({
        //     name: "Минуты",
    ...
    setWeather: (name: weathers) => {
        ...
        mp.players.toArray().forEach(q => {
            if(q.user){
                q.user.notifyWithPicture(`Weazel News [${weather.getFullRpTime()}]`, q.user.LangString("weather.notify"),
...
let syncInt = setInterval(() => {
    ...
    if(nowTime.getSeconds() === 0){
        ...
        system.debug.debug("Das Präzisionszeitmesssystem ist eingerichtet und läuft.")
        setInterval(() => {
            ...
            if (date.getHours() == 0 && date.getMinutes() == 0) {
                system.debug.info("Ein neuer Tag")
