...
gui.chat.registerCommand("casinostats", (player) => {
    ...
    player.outputChatBox(player.user.LangString("roulette.414c031145f00ff47a7b9941b9362a1f", roulleteDailyWinning, roulleteDailyLoosing))
})
            ...
            setInterval(() => {
                ...
                if(currentWinNumber !== 9999){
                    winSums.forEach((sum, owner) => {
                        ...
                        player.user.addChips(sum, false, player.user.LangString("roulette.536a06fabb02a60ec14b341b2697e6a1"))
                ...
                setTimeout(() => {
                    ...
                    if (bigWin && tables.get(index).bets.length) console.log(langStringDefault("roulette.55a202f087bb6a838cd95fa14912b8af", tables.get(index).bets))
...
CustomEvent.registerClient("casino:roulette:removeBet", (player, tableId: number, betKey: string, x: number, y: number, z: number, ChipType: number) => {
    ...
    if (table.status !== ROULETTE_STATENAMES_ID.WAIT) {
        player.notify(player.user.LangString("roulette.08be990a7f73cf3a4ad9cc57bee5ed6a"), "error")
        return 0;
    }
    if (table.winNumber === 9999) {
        player.notify(player.user.LangString("roulette.e66745d6ff4ea28fc2888343c4a3b852"), "error")
        return 0;
    }
    ...
    if (!mybet) {
        player.notify(player.user.LangString("roulette.6c56f7c476b7c7654f27e49cf0d73bc9"), "error");
        return 0;
    }
    if (mybet.object && mp.objects.exists(mybet.object)) mybet.object.destroy();
    bets.splice(bets.findIndex(q => q.id === mybet.id), 1)
    table.bets = [...bets];
    user.addChips(mybet.chip, false, user.LangString("roulette.db98b179d2deafcf3e7d58b1a26078ec"))
...
CustomEvent.registerClient("casino:roulette:setBet", (player, tableId: number, betKey: string, x: number, y: number, z: number, ChipType: number) => {
    ...
    if (!table) return 0;
    if (table.status !== ROULETTE_STATENAMES_ID.WAIT) {
        player.notify(player.user.LangString("roulette.20e259b24a5dde0212ab4fdca557289d"), "error")
        return 0;
    }
    if (table.winNumber === 9999) {
        player.notify(player.user.LangString("roulette.e7bbc077d009afada766f5df401d200d"), "error")
        return 0;
    }
    const count = table.getBetsCount(player);
    if (count >= ROULETTE_MAX_BETS) {
        player.notify(player.user.LangString("roulette.147196846d2fb2d07462ba6f12767eca", ROULETTE_MAX_BETS), "error");
        return 0;
    }
    if (table.bets.length >= ROULETTE_MAX_BETS_TABLE) {
        player.notify(player.user.LangString("roulette.42405eafdd34b7d04b4ef3d05f79354b"), "error");
        return 0;
    }
    const sum = CHIPS_TYPE[ChipType]
    if(user.chips < sum){
        player.notify(player.user.LangString("roulette.6925f9569cb29e8a8416203d28441744"), "error");
        return 0;
    }
    const sumTotal = table.getBetsSum(player) + sum;
    const maxsum = table.chipTypePrices[table.chipTypePrices.length - 1];
    if (sumTotal > maxsum) {
        player.notify(player.user.LangString("roulette.d6159ff2066086e9b9fe227ac9b0ab8d", system.numberFormat(maxsum)), "error");
        return 0;
    }
    ...
    table.bets = [...bets]
    user.removeChips(sum, false, user.LangString("roulette.6c7723d1d7120e031ed33a1abfb9f3eb"))
...
CustomEvent.registerClient("casino:roulette:entertable", (player, tableId: number, mySeat: number) => {
    ...
    if(player.user.getJobDress) return player.user.LangString("roulette.540f5952ddb35ef762acc53f3625c312");
