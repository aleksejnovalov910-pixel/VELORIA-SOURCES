...
export class PokerGameLobby {
    ...
    public async addPlayer(player: PlayerMp): Promise<void> {
        if (player.user.chips < this._game.settings.smallBlind * 2) {
            return player.notify(player.user.LangString("pokerGameLobby.5d692760d56d4d5c0573cedbd53eccfe"), "warning")
        }

        const response = await menu.input(
            player,
            player.user.LangString("pokerGameLobby.df73f3e1c3e8a960a9c51ee79aad55e1", player.user.chips),
            0,
        ...
        if (isNaN(response) || response < 0 || response > 10 * this._game.settings.smallBlind * 2) {
            return player.notify(player.user.LangString("pokerGameLobby.0147856e80ad11133a3e8ce5a55d4a15"), "warning")
        }

        player.user.removeChips(response, false, player.user.LangString("pokerGameLobby.d67f3c98049f19de55cc41662336cb9c"))
    ...
    public async removePlayer(player: PlayerMp): Promise<void> {
        ...
        player.user.addChips(removedPokerPlayer[0].balance, false, player.user.LangString("pokerGameLobby.049f5a79bd33c72976820349e2b0f98e"))
