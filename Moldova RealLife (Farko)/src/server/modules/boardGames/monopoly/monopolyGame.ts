...
export class MonopolyGame {
    ...
    private createColshape() {
        colshapes.new(system.getVector3Mp(this._settings.pos[0]), player => player?.user?.LangString("monopolyGame.d8526499942310b5cdd4a0f3571b7ba7") ?? langStringDefault("monopolyGame.d8526499942310b5cdd4a0f3571b7ba7"), player => {
    ...
    public onPlayerSellFirms(playerMp: PlayerMp, firmIds: number[]) {
        const player = this.players.find(p => p.player == playerMp)
        if (this._currentPlayer != player) return playerMp.notify(playerMp.user.LangString("monopolyGame.b88f0a02b03d8defae9916455ad71f20"), "error")
    ...
    public addPlayer(player: PlayerMp) {
        if (this._started) return player.notify(player.user.LangString("monopolyGame.855aa75e799866bd10f2ab4fba95a268"), "error")
