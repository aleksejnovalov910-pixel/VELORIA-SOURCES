...
export class IslandBattle {
    ...
    private intervalHandler = () => {
        ...
        if (this.state === ISLAND_BATTLE_STATE.PREPARE && !(this.ticks % 5)) {
            mp.players.toArray().filter(player =>
                ...
                .forEach(p => {
                    p.notify(
                        p.user.LangString("islandBattle.9c0b6cf178686981025183f615bca036", TICKS_FOR_BATTLE_START - this.ticks),
                        "info",
                        ISLAND_BATTLE_NOTIFY_IMAGE
                    );
                })
        ...
        if (winner.length === 1) {
            ...
            this.fractions.forEach((fraction, key) => {
                fraction.finishBattle(
                    key === winner[0] ?
                        langStringDefault("islandBattle.aebe4fd7aa0525b5dbfc1b28b073ed8a")
                        :
                        langStringDefault("islandBattle.b44fc8fa33e998f2a6a348f8fca45ad9"));
            })
        } else {
            this.fractions.forEach((fraction, key) => {
                if (winner.includes(fraction.config.id)) return;
                fraction.finishBattle(langStringDefault("islandBattle.1973f063561bf161e0c1ee3e0962fcce"));
            ...
            this.battleNotify(langStringDefault("islandBattle.18f4f3be423e7c8a4e0293fa69f266d2"), "info");
        }
    }
    ...
    public onChangeOwner = (fractionId: number, id: number) => {
        ...
        this.battleNotify(langStringDefault("islandBattle.1903707b877c29a381a4263b2f368707", cfg.fullName, id), "info");
    }
    ...
    public onPointInteract = (fractionId: number, id: number) => {
        ...
        this.battleNotify(langStringDefault("islandBattle.1aff6242fadca61b21ca645b37ac276c", cfg.fullName, id), "warning");
    }
    ...
    protected notifyPlayer(player: PlayerMp) {
        player.notify(player.user.LangString("islandBattle.ebde62befa21058da9ef7479d50352f8"),
