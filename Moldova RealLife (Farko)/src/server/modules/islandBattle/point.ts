...
export class Point {
    ...
    constructor(id: number, config: IIslandPoint, onChangeOwner: Function, onPointInteract: Function) {
        ...
        this.blip = mp.blips.new(
            ...
            config.pos,
            {
                color: 4,
                dimension: ISLAND_BATTLE_DIMENSION,
                name: langStringDefault("point.412da3f49bbd223bd28e6949ab2aa519", this.id),
                shortRange: true,
            }
        )

        this.shape = colshapes.new(this.config.pos, player => player?.user?.LangString("point.ce08c86acb24f27a0d6dae8247dd3292", this.id) ?? langStringDefault("point.ce08c86acb24f27a0d6dae8247dd3292", this.id), this.shapeHandler, {
            dimension: ISLAND_BATTLE_DIMENSION,
            type: 1,
            color: [0, 0, 0, 125],
            radius: 2
    ...
    private shapeHandler = (player: PlayerMp) => {
        ...
        if (this.owner === player.user.fraction) return player.notify(
            player.user.LangString("point.adad768b1764880294f95b77f50ae04e"),
            "error",
            ISLAND_BATTLE_NOTIFY_IMAGE
        );

        if (this.busy !== null)
            return player.notify(player.user.LangString("point.ff3d6351c17f8abb50abce6b94ca9355"), "error",
                ISLAND_BATTLE_NOTIFY_IMAGE);
