...
export class Team {
    ...
    private enterShapeHandler = (player: PlayerMp, shape: ColshapeMp) => {
        ...
        if (player.vehicle) return player.notify(player.user.LangString("team.31864311dd9ca76d346900720d4f20c3"), "error");

        if (this.players.size >= ISLAND_BATTLE_MAX_COMMAND_PLAYERS)
            return player.notify(player.user.LangString("team.d6490e5271c1d653dbc3536e37302762"), "error",
                ISLAND_BATTLE_NOTIFY_IMAGE);

        this.players.set(player.user.id, player);
        player.notify(player.user.LangString("team.6eac1e72c8c56e86d9e96e45064ea71f"), "success",
            ISLAND_BATTLE_NOTIFY_IMAGE);
    }
    ...
    private leaveShapeHandler = (player: PlayerMp, shape: ColshapeMp) => {
        ...
        player.notify(player.user.LangString("team.4f81285a605857ceed288e04f8b3c54a"),
