...
class FractionsManager {
    ...
    private async init() {
        ...
        console.log(langStringDefault("main.8212a0efd260f8bfe85f8cc786e0c0a2", this.fractionsList.length))
    }
...
CustomEvent.registerCef("tablet:setRanks", (player: PlayerMp, ranks: IFractionRank[]) => {
    ...
    if (!timeouts.has(player.user.fraction) || system.timestamp - timeouts.get(player.user.fraction) > 360) {
        ...
        player.notify(player.user.LangString("main.56a4bbc9e09080d1735ee5cf221ad06b"), "success");
        player.user.log("tabletFraction", langStringDefault("main.4826e6fa3076fc778546199b94db6e98", player.user.fraction))

    }else{
        player.notify(player.user.LangString("main.ed5b102af3d66366ef4f1e5befdf5777"), "error");
    }
})
