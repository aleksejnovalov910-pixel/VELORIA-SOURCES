...
export class UserQuest extends UserAddonClass {
        ...
        questes.map((quest, index) => {
            if (!quest[2]) {
                ...
                if (!cfg || cfg.tasks.length !== quest[1].length) {
                    ...
                    system.debug.debug(langStringDefault("quest.ce3f9268affa631ad22b24be4156acdb", this.name, this.id, quest[0], JSON.stringify(quest)))
                }
            }
        })
        ids.map(id => {
    ...
    setQuestComplete = (id: number) => {
        ...
        s[2] = true;
        this.quests = q;
        if (!cfg.botData) this.player.notify(langStringDefault("quest.62e7613931daa5d90b634a68187b24fe", cfg.name), "success");
        cfg.reward.map(reward => {
            if (reward.type === "exp") this.giveExp(reward.value);
            else if (reward.type === "cash") this.addMoney(reward.value, true, this.user.LangString("quest.b7bcd6e9af83363ae4bac968c45405c0", cfg.name, cfg.id));
            else if (reward.type === "bank") this.addBankMoney(reward.value, true, this.user.LangString("quest.20ca2acb6658227ea586f7505e09dc5b", cfg.name, cfg.id), cfg.botData ? quests.getBot(cfg.botData.id).name : langStringDefault("quest.181170d7d7f74d97febee5c9e6aea104", cfg.name));
