...
export class UserWeapon extends UserAddonClass {
    ...
    reloadCurrentWeapon = async (auto = true)=> {
        ...
        if (!currentWeapon) {
            if(!auto) this.player.notify(this.player.user.LangString("weapon.52db9ab3e311f013a6ac56df6dac05d5"), "error");
            return
        }
        ...
        if (!item) {
            if(!auto) this.player.notify(this.player.user.LangString("weapon.65085ed03ddc88ad7a945f594d0882bd"), "error")
            return
        }
        const useCount = Math.min(max, item.count);
        if(useCount <= 0) {
            if(!auto) this.player.notify(this.player.user.LangString("weapon.72c16357bd793c88307a8231eefe3506"), "error");
            return
        }
