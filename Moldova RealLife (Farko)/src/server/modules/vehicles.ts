...
import {
	OPENING_HOOD_LIST,
	PLAYER_IN_VEHICLE_CONFIG,
	PlayerInVehicleConfigItem,
} from "../../shared/player.in.vehicle";
...
CustomEvent.registerCef(
	...
	(
		player,
		data: { ids: number; status: boolean; angle: number; speed: number },
	) => {
		...
		veh.setVariable("driftAngle", data.angle);
		veh.setVariable("driftSpeed", data.speed);
		veh.setVariable("driftEnable", data.status);
	},
);
...
CustomEvent.registerCef("vehicle:requestrespawn", (player, id: number) => {
	...
	if (veh.data.carSharing)
		return player.notify(user.LangString("vehicles.requestrespawn.carSharing"));

	if (veh.familyOwner) {
		const familyVehicle = user.family.cars.find((q) => q.id == id);

		if (!familyVehicle || !familyVehicle.exists) return;

		if (!user.family.isCan(user.familyRank, "car_park"))
			return player.notify(user.LangString("vehicles.requestrespawn.family"));
	}
	...
	menu
		.accept(
			...
			user.LangString(
				!inSpawn
					? `vehicles.requestrespawn.accept1`
					: "vehicles.requestrespawn.accept2",
				system.numberFormat(inSpawn ? DELIVER_COST : VEHICLE_RESPAWN_COST),
			),
			"small",
		)
		.then(async (status) => {
			...
			if (inSpawn) {
				if (
					veh.familyOwner &&
					!user.family.isCan(user.familyRank, "car_teleport")
				)
					return player.notify("Nu ai permisiunea");
				if (player.dimension)
					return player.notify(
						user.LangString("vehicles.requestrespawn.err.dimension"),
						"error",
					);
				if (user.attachedToPlace)
					return player.notify(
						user.LangString("vehicles.requestrespawn.err.attachedToPlace"),
						"error",
					);
				if (user.bank_money < DELIVER_COST)
					return player.notify(
						user.LangString("vehicles.requestrespawn.err.money"),
					);
				if (user.inSaveZone)
					return player.notify(
						user.LangString("vehicles.requestrespawn.err.savezone"),
						"error",
					);
				if (orderCd.has(user.id) && orderCd.get(user.id) - system.timestamp > 0)
					return player.notify(
						user.LangString(
							"vehicles.requestrespawn.notify_1",
							orderCd.get(user.id) - system.timestamp > 0
								? orderCd.get(user.id) - system.timestamp
								: user.LangString("vehicles.requestrespawn.notify_2"),
						),
						"error",
					);
				if (player.vehicle)
					return player.notify(
						user.LangString("vehicles.requestrespawn.veh"),
						"error",
					);
				player.notify(
					user.LangString("vehicles.requestrespawn.veh.not"),
					"success",
				);
				if (
					!(await user.waitTimer(
						10,
						DELIVER_SECONDS,
						user.LangString("vehicles.requestrespawn.timer"),
					))
				)
					return;
				if (player.vehicle)
			...
			if (veh.onParkingFine)
				return player.notify(
					user.LangString("vehicles.requestrespawn.onParkingFine"),
					"error",
				);
			if (!veh.exists) return;
			if (veh.vehicle.getOccupants().length)
				return player.notify(
					user.LangString("vehicles.requestrespawn.occupants"),
					"error",
				);
			if (
				!user.tryRemoveBankMoney(
					inSpawn ? DELIVER_COST : VEHICLE_RESPAWN_COST,
					true,
					user.LangString(
						!inSpawn
							? `vehicles.requestrespawn.removeMoneyReason1`
							: "vehicles.requestrespawn.removeMoneyReason2",
						veh.name,
						veh.number,
					),
					user.LangString("vehicles.requestrespawn.removeMoneyWho"),
				)
			)
				return;
			if (inSpawn) {
				...
				Vehicle.teleport(
					veh.vehicle,
					new mp.Vector3(myPos.x, myPos.y, myPos.z),
					0,
					0,
				);
			...
			player.notify(
				user.LangString(
					inSpawn
						? `vehicles.requestrespawn._1_1`
						: "vehicles.requestrespawn._1_2",
				),
				"success",
			);
		});
...
export class Vehicle {
	...
	static getInventory(vehicle: VehicleMp) {
		...
		if (vehicle.garagecarid) {
			...
			return inventory.getInventory(
				...
				vehicle.inventoryTmp,
			);
		else return inventory.getInventory(OWNER_TYPES.VEHICLE, vehicle.dbid);
	}
	static getName(vehicle: VehicleMp | string) {
		if (typeof vehicle !== "string" && !mp.vehicles.exists(vehicle))
			return null;
		const cfg = this.getVehicleConfig(
			typeof vehicle !== "string" ? vehicle.modelname : vehicle,
		);
	...
	static spawnNpcVehicles(test = false) {
		...
		const vehs = Vehicle.toArray()
			.filter((q) => q.dimension === 0)
			.map((q) => q.position)
			.filter((q) => system.isPointInPoints(q, points, 50));
		points = points.filter((q) => !system.isPointInPoints(q, vehs, 50));

		points.map((item, index) => {
			const model =
				item.models && item.models.length > 1
					? system.randomArrayElement(item.models)
					: system.randomArrayElement(npcVehicles);
	...
	static addBlockNpcCarZone(pos: Vector3Mp) {
		Vehicle.toArray()
			.filter((veh) => veh.npc)
			.filter((veh) => system.distanceToPos(veh.position, pos) < 30)
			.map((veh) => veh.destroy());
	...
	setOwner(
		...
		if (owner) {
			...
			if (this.exists && this.vehicle.getVariable("ownerfamily"))
				this.vehicle.setVariable("ownerfamily", null);
			this.save();
		}
	}
	...
	setOwnerFamily(
		...
		if (owner) {
			...
			if (this.exists && this.vehicle.getVariable("owner"))
				this.vehicle.setVariable("owner", null);
			this.save();
		}
	}
	get owner() {
	...
	constructor(data: VehicleEntity) {
		...
		let pos = JSON.parse(data.position) as {
			x: number;
			y: number;
			z: number;
			h: number;
		...
		if (this.data && this.data['engineHealth'] && typeof this.data?.engineHealth === "number" && this.data.engineHealth != -1) {
			this.vehicle.setVariable('engineHealth', this.data.engineHealth);
			// this.vehicle.setEngineHealth(this.data.engineHealth);
		}
	...
	get haveDriftMode() {
		return (
			(this.exists && !!this.vehicle.getVariable("driftAdmin")) ||
			!!this.data.tuning.find(
				(m) =>
					m[0] === 3004 &&
					m[1] !== lscConfig.find((q) => q.id === 3004).default,
			)
		);
	}
	...
	applyCustomization() {
		...
		let tuning = this.data && this.data?.tuning ? [...this.data.tuning] : [];
		lscConfig.map((mod) => {
			if (mod.default != null && !this.data?.tuning.find((m) => m[0] == mod.id))
				tuning.push([mod.id, mod.default]);
		});

		tuning.map(([mod, value]) => {
			if (mod == 1000 || mod == 1001 || mod == 3002) return;
			else if (mod < 0 || mod > 61 || mod == 22)
				return toSync.push([mod, value]);
			else this.vehicle.setMod(mod, value);
		});
		this.vehicle.setVariable(
			"neonColor",
			JSON.parse(this.data.color_neon) as RGB,
		);
		this.vehicle.setVariable(
			"primaryColor",
			JSON.parse(this.data.color_primary) as RGB,
		);
		this.vehicle.setVariable(
			"secondaryColor",
			JSON.parse(this.data.color_secondary) as RGB,
		);
		this.vehicle.setVariable(
			"tyreSmokeColor",
			JSON.parse(this.data.color_tyre_smoke) as RGB,
		);
		...
		Vehicle.setPrimaryColor(
			this.vehicle,
			primaryColor[0],
			primaryColor[1],
			primaryColor[2],
		);
		Vehicle.setSecondaryColor(
			this.vehicle,
			secondaryColor[0],
			secondaryColor[1],
			secondaryColor[2],
	...
	respawn(withRepair: boolean = true) {
		if (mp.vehicles.exists(this.vehicle)) {
			if (this.onParkingFine) {
				...
				setTimeout(() => {
					...
					withRepair && Vehicle.repair(this.vehicle, true);
					if (this.exists) this.vehicle.usedAfterRespawn = false;
				}, 2000);
			}
			if (this.vehicle.getOccupants().length === 0)
				this.vehicle.usedAfterRespawn = false;
		}
	}
	...
	get sellSumMoney() {
		...
		let sum = this.isDonate
			? this.data.cost * DONATE_VIRTS_PER_COIN
			: this.data.cost;
	...
	sellVehicleByMoney() {
		...
		if (this.familyOwner) {
			...
			family.addMoney(
				...
				null,
				langStringDefault(
					"vehicles.sellVehicleByMoney.family",
					this.name,
					this.number,
				),
			);
	...
	async selectParkPlace(
		player: PlayerMp,
	): Promise<{ type: "house" | "parking"; id: number }> {
		const user = player.user;
		if (this.familyOwner && user.familyId === this.familyOwner)
			return Vehicle.selectParkPlace(player, this.avia, true);
	...
	static selectParkPlace(
		player: PlayerMp,
		avia: boolean,
		isFamilyMoney: boolean = false,
		userEntity?: UserEntity,
	): Promise<{
		type: "house" | "parking";
		id: number;
		pos: { d: number; h: number; x: number; y: number; z: number };
	}> {
		const user = player.user;
		return new Promise((resolve) => {
			if (isFamilyMoney) {
				...
				if (!family) {
					player.notify(
						user.LangString("vehicles.selectParkPlace.family"),
						"error",
					);
					return resolve(null);
				}
				...
				if (!house) {
					player.notify(
						user.LangString("vehicles.selectParkPlace.house"),
						"error",
					);
					return resolve(null);
				}
				...
				if (!slot) {
					...
					return player.notify(
						user.LangString(
							avia
								? "vehicles.selectParkPlace.slotAvia"
								: "vehicles.selectParkPlace.slot",
						),
					);
				}
			...
			const m = menu.new(
				player,
				user.LangString("vehicles.selectParkPlace.menu.title"),
				user.LangString("vehicles.selectParkPlace.menu.subtitle"),
			);
			m.onclose = () => {
				resolve(null);
			};
			...
			if (houseEnt && (houseEnt.car_interrior || avia)) {
				m.newItem({
					...
					onpress: () => {
						if (!houseEnt.car_interrior)
							return player.notify(
								user.LangString("vehicles.selectParkPlace.menu.select.err"),
								"error",
							);
						...
						if (!slot)
							return player.notify(
								user.LangString(
									avia
										? "vehicles.selectParkPlace.menu.select.err.slotAvia"
										: "vehicles.selectParkPlace.menu.select.err.slot",
								),
							);
						m.close();
				...
				m.newItem({
					name: houseEnt
						? `${houseEnt.name} #${houseEnt.id}`
						: user.LangString("vehicles.selectParkPlace.menu.select.house"),
					more: user.LangString("vehicles.selectParkPlace.menu.select.more"),
					desc: user.LangString("vehicles.selectParkPlace.menu.select.desc"),
				});
			}

			const allVehs = parking.allVehsInAllParking();
			const myCarsOnParks = allVehs.filter(
				(veh) => veh.entity.owner === (userEntity ? userEntity.id : user.id),
			).length;
			if (
				myCarsOnParks >=
				(userEntity ? User.parkingMax(userEntity) : user.parkingMax)
			) {
				m.newItem({
					name: user.LangString("vehicles.selectParkPlace.menu.1"),
					desc: user.LangString("vehicles.selectParkPlace.menu.1.desc"),
				});
			} else {
				business.data
					.filter(
						(q) =>
							q.type === BUSINESS_TYPE.PARKING && (q.sub_type === 1) === avia,
					)
					.map((biz) => {
						let dims: number[] = [];
						...
						m.newItem({
							...
							desc: user.LangString(
								"vehicles.selectParkPlace.menu.2.desc",
								biz.name,
								system.numberFormat(PARKING_START_COST),
								system.numberFormat(PARKING_DAY_COST),
							),
							more: user.LangString(
								"vehicles.selectParkPlace.menu.2.more",
								PARKING_CARS.length * dims.length -
									allVehs.filter((q) => dims.includes(q.entity.position.d))
										.length,
							),
							onpress: () => {
								...
								if (!slot)
									return player.notify(
										user.LangString("vehicles.selectParkPlace.menu.2.errslot"),
										"error",
									);
								m.close();
								return resolve({
	...
	get spawnPointType() {
		return system.isPointInPoints(this.position, PARKING_CARS, 5)
			? "parking"
			: "house";
	}
	get inSpawnPoint() {
	...
	static inSpawnPoint(vehicle: VehicleMp) {
		...
		if (system.distanceToPos(vehicle.position, vehicle.spawnPosition) > 2)
			return false;
		return true;
	}
	...
	static respawn(vehicle: VehicleMp, onlyRespawn = false) {
		...
		if (vehicle.spawnPosition) {
			Vehicle.teleport(
				...
				vehicle.spawnRotation ? vehicle.spawnRotation.z : vehicle.rotation.z,
				typeof vehicle.spawnDimension === "number"
					? vehicle.spawnDimension
					: vehicle.dimension,
			);
		}
	}
	...
	static findFreeParkingZone(pos: Vector3Mp, r: number, dimension = 0) {
		let points = vehicleSpawnPoints
			.filter((q) => q && q.x && q.y && q.z && q.heading)
			.map((q) => {
				return { ...q, free: true };
			});
		...
		const vehs = Vehicle.toArray()
			.filter(
				(q) =>
					...
					system.distanceToPos(q.position, pos) <= r,
			)
			.map((q) => q.position);
		points.map((q) => {
			...
			if (
				!!inventory
					...
					.find(
						(item) =>
							item.item_id === 805 &&
							item.advancedNumber === vehicle.entity.key,
					)
			)
				return true;
		}
	...
	static teleport(
		...
		setTimeout(() => {
			...
			vehicle.position = position;
			if (typeof h === "number")
				vehicle.rotation = new mp.Vector3(
					vehicle.rotation.x,
					vehicle.rotation.y,
					h,
				);
	...
	static getPlayerVehicles(id: number) {
		return Vehicle.toArray()
			.filter(
				(veh) => veh.dbid && veh.entity && veh.getVariable("owner") === id,
			)
			.map((veh) => {
				return veh.entity;
			});
	...
	static getVehiclesByPlayerKeys(player: PlayerMp): Vehicle[] {
		const carKeys = player.user.allMyItems.filter(
			(item) => item.item_id === KEYS_ITEM_ID && item.advancedString === "car",
		);
	...
	static getByKey(key: number) {
		return Vehicle.toArray().find(
			(vehicle) => vehicle.dbid && vehicle.entity && vehicle.entity.key === key,
		)?.entity;
	}
	...
	static getFamilyVehicles(id: number) {
		return Vehicle.toArray()
			.filter(
				(veh) =>
					veh.dbid && veh.entity && veh.getVariable("ownerfamily") === id,
			)
			.map((veh) => {
				return veh.entity;
			});
	...
	static addFuel(vehicle: VehicleMp, value: number) {
		...
		vehicle.setVariable(
			"fuel",
			Math.min(Vehicle.getFuel(vehicle) + value, Vehicle.getFuelMax(vehicle)),
		);
	}
	...
	static spawnPlayersVehicles() {
		return VehicleEntity.find().then((vehicles) =>
			vehicles.map((vehicle) => Vehicle.spawnPlayerVehicle(vehicle)),
		);
	}
	static spawnPlayerVehicle(vehicle: VehicleEntity) {
		return new Vehicle(vehicle);
	}
	static spawnFractionVehicle(
		fraction: number,
		garagecarid: number,
		garage: number,
		model: string,
	...
	static createNewDatabaseVehicle(
		player: PlayerMp,
		cnfId: number,
		...
		return new Promise((resolve, reject) => {
			...
			v.color_primary = JSON.stringify([
				primaryColor.r,
				primaryColor.g,
				primaryColor.b,
			]);
			v.color_secondary = JSON.stringify([
				secondaryColor.r,
				secondaryColor.g,
				secondaryColor.b,
			]);
			...
			v.save()
				.then((v1) => {
					if (isFamilyMoney) {
						system.debug.info(
							`Ein neues Fahrzeug wurde für die Familie erstellt ${v.family.name} ${conf.model}`,
						);
					} else {
						system.debug.info(
							`Es wurde ein neues Fahrzeug eingerichtet für ${player.user.name} #${player.dbid} ${conf.model}`,
						);
					}
	...
	static isVehicleCommercial(item: string | VehicleMp) {
		if (typeof item === "string")
			return commercialVehicles.includes(item.toLowerCase());
	...
	static hasAccessToVehicle(player: PlayerMp, veh: VehicleMp): boolean {
		...
		if (!haveAccess) {
			...
			if (
				user.fraction &&
				user.fraction === veh.fraction &&
				(!veh.fractionMinRank || user.rank >= veh.fractionMinRank)
			)
				haveAccess = true;
		...
		if (
			!haveAccess &&
			veh.entity &&
			veh.entity.familyOwner &&
			user.familyId &&
			user.familyId == veh.entity.familyOwner &&
			user.family.ranks.findIndex((r) => r.id == user.familyRank) >=
				user.family.ranks.findIndex((r) => r.id == veh.entity.fromRank)
		)
			haveAccess = true;
		...
		if (!haveAccess) {
			...
			if (veh.dbid && !veh.entity?.rentedBy) {
				if (veh.entity.owner === player.dbid) haveAccess = true;
					...
					!!user.allMyItems.find(
						(item) =>
							item.item_id === 805 &&
							item.advancedNumber === veh.entity.key &&
							item.advancedString === "car",
					)
				)
					haveAccess = true;
			}
			...
			if (
				veh.getVariable("busman") &&
				player.user.id === veh.getVariable("busman")
			)
				haveAccess = true;

			if (
				veh.getVariable("electrician") &&
				player.user.id === veh.getVariable("electrician")
			)
				haveAccess = true;

			if (
				veh.getVariable("sanitation") &&
				player.user.sanitationSquad === veh.getVariable("sanitation")
			)
				haveAccess = true;
		}
	...
	static lockVehStatusChange(player: PlayerMp, veh: VehicleMp) {
		...
		if (!mp.vehicles.exists(veh))
			return player.notify(
				user.LangString("vehicles.lockVehStatusChange.noveh"),
				"error",
			);
		if (veh.dimension != player.dimension)
			return player.notify(
				user.LangString("vehicles.lockVehStatusChange.dimension"),
				"error",
			);
		if (system.distanceToPos2D(veh.position, player.position) > 200)
			return player.notify(
				user.LangString("vehicles.lockVehStatusChange.dist"),
				"error",
			);
		let haveAccess = this.hasAccessToVehicle(player, veh);
		if (!haveAccess) {
			player.notify(
				user.LangString("vehicles.lockVehStatusChange.access"),
				"error",
			);
		} else {
			let newLockStatus = !Vehicle.getLocked(veh);
			Vehicle.setLocked(veh, newLockStatus);
			player.notify(
				user.LangString(
					newLockStatus
						? "vehicles.lockVehStatusChange.locked"
						: "vehicles.lockVehStatusChange.unlocked",
				),
				"success",
			);
		}
		...
		if (!user.walkingWithObject)
			player.user.playAnimation(
				[["anim@mp_player_intmenu@key_fob@", "fob_click_fp", 1]],
				true,
			);
	}
	...
	static haveTruck(data: string | VehicleMp): PlayerInVehicleConfigItem {
		return PLAYER_IN_VEHICLE_CONFIG.find(
			(param) =>
				param.model === (typeof data === "string" ? data : data.modelname),
		);
	}
	...
	static haveHood(data: string | VehicleMp): boolean {
		return !!OPENING_HOOD_LIST.find(
			(param) => param === (typeof data === "string" ? data : data.modelname),
		);
	}
...
CustomEvent.registerClient("admins:vehicle:color", (player) => {
	...
	let m = menu.new(
		player,
		player.user.LangString("vehicles.admins.color.menu.title"),
		player.user.LangString("vehicles.admins.color.menu.subtitle"),
	);
	const primaryColor = player.vehicle.getColorRGB(0);
	const secondaryColor = player.vehicle.getColorRGB(1);
	m.newItem({
		name: player.user.LangString("vehicles.admins.color.menu.1"),
		type: "color",
	...
	m.newItem({
		name: user.LangString("vehicles.admins.color.menu.2"),
		type: "color",
	...
	m.newItem({
		name: user.LangString("vehicles.admins.color.menu.3"),
		type: "range",
		rangeselect: [-1, 260],
...
CustomEvent.registerClient("admins:vehicle:info", async (player) => {
	...
	let m = menu.new(
		player,
		user.LangString("vehicles.admins.info.menu.title"),
		user.LangString("vehicles.admins.info.menu.subtitle"),
	);
	if (veh.entity) {
		m.newItem({
			name: user.LangString("vehicles.admins.info.menu.1"),
			more: user.LangString("vehicles.admins.info.menu.1.more"),
		});
		if (veh.entity.owner) {
			const owner = await User.getData(veh.entity.owner);
			if (owner) {
				m.newItem({
					name: user.LangString("vehicles.admins.info.menu.2"),
					more: `${owner.rp_name} ${owner.id}`,
					desc: user.LangString(
						"vehicles.admins.info.menu.2.desc",
						veh.entity.id,
					),
					onpress: () => {
			...
			if (owner) {
				m.newItem({
					name: user.LangString("vehicles.admins.info.menu.3"),
					more: `${owner.name} ${owner.id}`,
					desc: user.LangString(
						"vehicles.admins.info.menu.3.desc",
						veh.entity.id,
					),
				});
			}
		...
		m.newItem({
			name: user.LangString("vehicles.admins.info.menu.4"),
			onpress: () => {
				Logs.open(
					player,
					`vehicle_${veh.entity.id}`,
					`KFR ${veh.entity.name} #${veh.entity.id}`,
				);
			},
		});
	} else if (veh.npc) {
		m.newItem({
			name: user.LangString("vehicles.admins.info.menu.5"),
			more: `NPC`,
			desc: user.LangString("vehicles.admins.info.menu.5.desc"),
		});
	} else if (veh.fraction) {
		m.newItem({
			name: user.LangString("vehicles.admins.info.menu.6"),
			more: user.LangString("vehicles.admins.info.menu.6.more", veh.fraction),
		});
	}
	m.newItem({
		name: user.LangString("vehicles.admins.info.menu.7"),
		more: `${veh.modelname}`,
	});
	m.newItem({
		name: user.LangString("vehicles.admins.info.menu.8"),
		more: veh.numberPlate,
	});
	if (player.user.isAdminNow(5)) {
		const status = !!veh.getVariable("driftAdmin");
		m.newItem({
			name: user.LangString("vehicles.admins.info.menu.9"),
			more: user.LangString(
				status
					? "vehicles.admins.info.menu.9.status"
					: "vehicles.admins.info.menu.9.unstatus",
			),
			desc: `${!DRIFT_PARAMS.includes(veh.modelname) ? user.LangString("vehicles.admins.info.menu.9.desc") : ""}`,
			onpress: () => {
				...
				player.notify(
					user.LangString(
						!status
							? "vehicles.admins.info.menu.9.notify.1"
							: "vehicles.admins.info.menu.9.notify.2",
					),
					"success",
				);
			},
...
CustomEvent.registerClient("admins:vehicle:fuel", (player) => {
	...
	if (!player.user.isAdminNow()) return;
	const user = player.user;
	let m = menu.new(player, user.LangString("vehicles.admins.fuel.menu.title"));
	m.newItem({
		name: user.LangString("vehicles.admins.fuel.menu.1"),
		...
		onpress: () => {
			...
			menu
				.input(
					player,
					user.LangString("vehicles.admins.fuel.menu.1.input"),
					Vehicle.getFuel(player.vehicle),
					4,
					"int",
				)
				.then((val) => {
					...
					if (val < 0)
						return player.notify(
							user.LangString("vehicles.admins.fuel.menu.1.input.err"),
							"error",
						);
					if (val > Vehicle.getFuelMax(player.vehicle))
						return player.notify(
							user.LangString("vehicles.admins.fuel.menu.1.input.err.f"),
							"error",
						);
					Vehicle.setFuel(player.vehicle, val);
					player.notify(
						user.LangString("vehicles.admins.fuel.menu.1.input.notify"),
						"success",
					);
				});
		},
	...
	m.newItem({
		name: user.LangString("vehicles.admins.fuel.menu.2"),
		onpress: () => {
			if (!player.vehicle) return;
			if (!player.user.isAdminNow()) return;
			Vehicle.setFuel(player.vehicle, Vehicle.getFuelMax(player.vehicle));
			player.notify(
				user.LangString("vehicles.admins.fuel.menu.2.notify"),
				"success",
			);
		},
	});
...
CustomEvent.registerClient(
	"admins:vehicle:respawn",
	(player, id: number, onlyRespawn: boolean) => {
		if (!player.user.isAdminNow()) return;
		const vehicle = mp.vehicles.at(id);
		if (!vehicle)
			return player.notify(
				player.user.LangString("vehicles.admins.noveh"),
				"error",
			);
...
CustomEvent.registerClient("admins:vehicle:fullFix", (player, id: number) => {
	...
	if (vehicle.getOccupants().length == 0 || vehicle.getOccupant(0) === player)
...
function checkForLevel(player: PlayerMp, vehicle: VehicleMp) {
	...
	// Daca vrei sa pastrezi exceptiile pentru motociclete si biciclete, le poti lasa:
	// if (isAMotorcycle(vehicle.modelname)) return;
	// if (isABike(vehicle.modelname)) return;

	// Scoatem complet verificarea pentru licente:
	// if (
	// 	player.user.haveActiveLicense("car") ||
	// 	player.user.haveActiveLicense("truck") ||
	// 	player.user.haveActiveLicense("moto")
	// )
	// 	return;

	// player.removeFromVehicle();
	// player.notify(player.user.LangString("vehicles.checkForLevel.notify"));
}
...
// 	// if (player.user.playtime >= 3) return;
	...
	mp.vehicles.forEachInDimension(0, (veh) => {
		...
		if (veh.rentCar) {
			...
			if (veh.afkTime >= maxAfkRentCarTime) veh.afkTime = 9999999;
				...
				(veh.afkTime % 5 === 0 ||
					[1, 2, 3, 4].includes(maxAfkRentCarTime - veh.afkTime)) &&
				veh.afkTime > 0
			) {
				const owner = mp.players
					.toArray()
					.find((target) => target.dbid === veh.rentCarOwner);
				if (owner) {
					owner.user.notifyPhone(
						owner.user.LangString("vehicles.rent.app"),
						owner.user.LangString("vehicles.rent.title"),
						owner.user.LangString(
							"vehicles.rent.text",
							Math.floor(maxAfkRentCarTime - veh.afkTime),
						),
						"error",
					);
				}
		...
		if (needResp) {
			if (veh.dbid) {
				...
				if (owner && mp.players.exists(owner)) {
					if (system.distanceToPos2D(owner.position, veh.position) < 100)
						return (veh.afkTime -= 5);
				}
			}
...
export const getVehicleConfig = (model: string): VehicleConfigsEntity => {
	let item = [...vehicleConfigs].find(
		([_, item]) => item.model.toLowerCase() == model.toLowerCase(),
	);
...
export const reloadConfig = (conf: VehicleConfigsEntity) => {
	Vehicle.toArray()
		.filter((veh) => veh.modelname.toLowerCase() == conf.model.toLowerCase())
		.map((veh) => {
			setVehicleParamsByConfig(veh, conf);
		});
};

export const setVehicleParamsByConfig = (
	vehicle: VehicleMp,
	conf?: VehicleConfigsEntity,
) => {
...
export const loadVehicleConfigs = () => {
	return new Promise((resolve) => {
		console.time("Transportkonfigurationen laden");
		VehicleConfigsEntity.find().then((list) => {
			list.map((item) => {
				vehicleConfigs.set(item.id, item);
			});
			console.timeEnd("Transportkonfigurationen laden");
			resolve(null);
		});
	});
};
...
CustomEvent.registerClient("engineveh", (player, vehId: number) => {
	...
	if (!veh)
		return player.notify(player.user.LangString("vehicles.engine"), "error");

	if (veh.brIndexUser) return;
	//if (veh.getOccupant(0) !== player) return player.notify("Вы должны находится за рулём", "error");
	if (!Vehicle.hasAccessToVehicle(player, veh))
		return player.notify(
			player.user.LangString("vehicles.engine.access"),
			"error",
		);
		...
		if (cfg.fuel_max > 0 && !Vehicle.getFuel(veh)) {
			return player.notify(
				player.user.LangString(
					cfg.fuel_type === VEHICLE_FUEL_TYPE.ELECTRO
						? "vehicles.engine.fuel.el"
...
setInterval(() => {
	mp.vehicles.forEachInDimension(0, (vehicle) => {
		...
		let resultRate = 0;
		if (fuel_type !== VEHICLE_FUEL_TYPE.ELECTRO)
			resultRate += VEHICLE_FUEL_RATE.AFK * fuel_min;
		if (speed > 0) {
...
gui.chat.registerCommand("eject", (player: PlayerMp, str: string) => {
	...
	if (!vehicle)
		return player.notify(
			player.user.LangString("vehicles.eject.vehicle"),
			"error",
		);
	if (!vehicle.getOccupant(0) || vehicle.getOccupant(0) !== player)
		return player.notify(
			player.user.LangString("vehicles.eject.getOccupant"),
			"error",
		);
	...
	if (!target)
		return player.notify(
			player.user.LangString("vehicles.eject.target"),
			"error",
		);
	if (!vehicle.getOccupants().find((el) => el.user.id === target.user.id))
		return player.notify(player.user.LangString("vehicles.eject.occ"), "error");
	if (target.user.id === player.user.id)
		return player.notify(
			player.user.LangString("vehicles.eject.sameID"),
			"error",
		);

	target.notify(target.user.LangString("vehicles.eject.ok"));
