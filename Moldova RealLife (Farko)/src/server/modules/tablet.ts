...
CustomEvent.registerClient("tablet:openTablet", async (player) => {
  ...
  if (!tabletItem) return player.notify("Nu ai tableta in inventar", "error");
...
export const tablet = {
    ...
    const vehicles = distinctArray(
      ...
      if (q.vehicle.usedAfterRespawn) {
        ...
        pos = [...FINE_CAR_POS].sort((a, b) => {
          return (
            system.distanceToPos(player.position, a) -
            system.distanceToPos(player.position, b)
          );
        })[0];
      }
      return {
  ...
  getBusinessData: async (player: PlayerMp) => {
    ...
    return bizData
      ? {
          ...
          logs: (
            await BusinessHistoryEntity.find({
  ...
  updateFamilyData: async (family: Family) => {
    SocketSyncWeb.fire(
      `tablet_family_${family.id}`,
      JSON.stringify(await tablet.getFamilyData(family))
    );
  },
  getFamilyData: async (family: Family) => {
    return {
      ...
      cargoNeedTime: family.lastCargoBattleWin
        ? FAMILY_TIMEOUT_AFTER_CARGO_BATTLE -
          Math.round((system.timestamp - family.lastCargoBattleWin) / 60)
        : 0,
      contract: (() => {
        let outputContracts = [] as {
          id: number;
          name: string;
          desc: string;
          status: number;
        ...
        Object.entries(family.contracts).map(([id, progress]) => {
          ...
          outputContracts.push({
            ...
            status:
              progress == -1
                ? progress
                : Math.floor((progress * 100) / contract.needScore),
            win: contract.win,
          });
        });
  ...
  gosSuspectsReload: () => {
    SocketSyncWeb.fire(
      `tablet_gosSuspects`,
      JSON.stringify(tablet.getGosSuspects())
    );
  },
  getGosSuspects: () => {
    return mp.players
      .toArray()
      .filter((q) => q.user && q.user.wanted_level)
      .map((player) => {
        return {
          id: player.user.id,
          name: player.user.name,
          wanted: player.user.wanted_level,
        };
      });
  },
  getFractionData: async (player: PlayerMp) => {
    ...
    return {
      ...
      mafiabiz: fractionCfg.getFraction(fraction)?.mafia
        ? business.data
            .filter((biz) => biz.mafiaOwner === fraction)
            .map((q) => {
              return {
                id: q.id,
                name: q.name,
      ...
      alerts: Dispatch.getFactionDispatches(fraction).map((d) => {
        return {
          id: d.id,
          name:
            mp.players.exists(d.fromPlayer) && d.fromPlayer.user
              ? d.fromPlayer.user.name
              : langStringDefault("tablet.unknown"),
...
CustomEvent.registerCef("tablet:gpsSwitch", (player) => {
  ...
  player.setVariable(
    "gpsTrack",
    !current ? `[${user.tag}] ${user.name} (${user.id})` : null
  );
  player.notify(
    player.user.LangString(
      current ? "tablet.gpsSwitch.1" : "tablet.gpsSwitch.2"
    ),
    "success"
  );
});
CustomEvent.registerCef("tablet:sendCode", (player, codeindex) => {
  ...
  User.notifyToFraction(
    user.fraction,
    `${fraction.name} [${user.name} (${user.id})]`,
    langStringDefault("tablet.sendCode"),
    fraction.codes[codeindex],
    "CHAR_BRYONY",
    10000
  );
});
CustomEvent.registerCef("tablet:sendTextFraction", (player, text: string) => {
  ...
  User.notifyToFraction(
    ...
    `${fraction.name} [${user.name} (${user.id})]`,
    langStringDefault(
      "tablet.sendTextFraction",
      fractionCfg.getRankName(user.fraction, user.rank)
    ),
    text,
    "CHAR_BRYONY",
    10000
...
CustomEvent.registerCef(
  ...
  (player: PlayerMp, storageId: number, itemId: number, rank: number) => {
    if (!player.user || !player.user.fraction) return;
    if (
      !fraction
        .getRightsForRank(player.user.fraction, player.user.rank)
        .includes(FRACTION_RIGHTS.STORAGE)
    )
      return;
    ...
    player.user.log(
      "tabletFraction",
      player.user.LangString("tablet.setRankForItem", player.user.fraction)
    );
...
CustomEvent.registerCef(
  "tablet:setRankForCar",
  (player: PlayerMp, garageId: number, carId: number, rank: number) => {
    if (!player.user || !player.user.fraction) return;
    if (
      !fraction
        .getRightsForRank(player.user.fraction, player.user.rank)
        .includes(FRACTION_RIGHTS.VEHICLES)
    )
      return;
...
CustomEvent.registerCef(
  ...
  (
    ...
    if (
      !fraction
        .getRightsForRank(player.user.fraction, player.user.rank)
        .includes(FRACTION_RIGHTS.BACK_CAR)
    )
      return;
    ...
    if (used) {
      const veh = Vehicle.toArray().find(
        (v) =>
          v.fraction === garage.fraction &&
          v.garage === garage.id &&
          v.garagecarid == car[2]
      );
      ...
      if (veh) {
        if (veh.getOccupants().length === 0) {
          ...
          player.user.log(
            ...
            player.user.LangString(
              "tablet.getBackCar",
              player.user.fraction,
              veh.modelname
            )
          );
          player.notify(
            player.user.LangString("tablet.getBackCar.notify"),
            "success"
          );
        } else {
          player.notify(
            player.user.LangString("tablet.getBackCar.notify2"),
            "error"
          );
        }
      }
      ...
      player.notify(
        player.user.LangString("tablet.getBackCar.notify3"),
        "success"
      );
    }
  }
...
CustomEvent.registerCef(
  "tablet:removeFractionWarn",
  (player: PlayerMp, id: number) => {
    if (!player.user || !player.user.fraction) return;
    if (
      !fraction
        .getRightsForRank(player.user.fraction, player.user.rank)
        .includes(FRACTION_RIGHTS.WARNS)
    )
      return;
    ...
    if (target) {
      ...
      if (data.rank >= user.rank)
        return player.notify(
          player.user.LangString("tablet.removeFractionWarn.rank")
        );

      if (data.fractionWarns === 0)
        return player.notify(
          player.user.LangString("tablet.removeFractionWarn.warns"),
          "error"
        );
      ...
      player.notify(
        player.user.LangString("tablet.removeFractionWarn.notify"),
        "success"
      );
      ...
      User.getData(id).then((data) => {
        ...
        if (data.rank >= user.rank)
          return player.notify(
            player.user.LangString("tablet.removeFractionWarn.rankerr")
          );

        if (data.fraction_warns === 0)
          return player.notify(
            player.user.LangString("tablet.removeFractionWarn.warnsErr"),
            "error"
          );
        ...
        player.notify(
          player.user.LangString("tablet.removeFractionWarn.data.notify"),
          "success"
        );
        player.user.log(
          "tabletFraction",
          player.user.LangString(
            "tablet.removeFractionWarn.data.log",
            data.id,
            data.fraction
          )
        );
...
CustomEvent.registerCef(
  ...
  (player: PlayerMp, id: number) => {
    ...
    if (target) {
      ...
      if (data.rank >= user.rank)
        return player.notify(
          player.user.LangString("tablet.giveFractionWarn.rank"),
          "error"
        );
      ...
      if (data.fractionWarns + 1 >= 3) {
        ...
        player.notify(
          player.user.LangString("tablet.giveFractionWarn.warns"),
          "success"
        );
        player.user.log(
          "tabletFraction",
          player.user.LangString(
            "tablet.giveFractionWarn.log",
            data.id,
            data.fraction
          )
        );
        ...
        player.notify(
          player.user.LangString("tablet.giveFractionWarn2.notify"),
          "success"
        );
        player.user.log(
          "tabletFraction",
          player.user.LangString(
            "tablet.giveFractionWarn2.log",
            data.id,
            data.fraction
          )
        );
      ...
      User.getData(id).then((data) => {
        ...
        if (data.rank >= user.rank)
          return player.notify(
            player.user.LangString("tablet.giveFractionWarn3.rank")
          );
        ...
        if (data.fraction_warns + 1 >= 3) {
          ...
          player.notify(
            player.user.LangString("tablet.giveFractionWarn3.notify"),
            "success"
          );
          player.user.log(
            "tabletFraction",
            player.user.LangString(
              "tablet.giveFractionWarn3.log",
              data.id,
              data.fraction
            )
          );
          ...
          player.notify(
            player.user.LangString("tablet.giveFractionWarn3.warns"),
            "success"
          );
          player.user.log(
            "tabletFraction",
            player.user.LangString(
              "tablet.giveFractionWarn3.logs",
              data.id,
              data.fraction
            )
          );
...
CustomEvent.registerCef("tablet:giveAward", (player: PlayerMp, id: number) => {
  ...
  if (
    !fraction
      .getRightsForRank(player.user.fraction, player.user.rank)
      .includes(FRACTION_RIGHTS.AWARDS)
  )
    return;
  ...
  if (!chest)
    return player.notify(player.user.LangString("tablet.giveAward.chest"));
  ...
  if (target) {
    ...
    if (
      ...
      return player.notify(player.user.LangString("tablet.giveAward.money"));
    ...
    target.user.addBankMoney(
      ...
      target.user.LangString(
        "tablet.giveAward.reason",
        target.user.fraction,
        target.user.rank
      ),
      ""
    );
    player.notify(player.user.LangString("tablet.giveAward.notify"), "success");
    player.user.log(
      "tabletFraction",
      player.user.LangString(
        "tablet.giveAward.log",
        data.id,
        data.fraction,
        data.rank
      )
    ...
    chest.addLog(
      player,
      langStringDefault(
        "tablet.giveAward.chestLog",
        system.timeStampString(system.timestamp, true),
        data.name,
        fraction.getAwardCount(target.user.fraction, target.user.rank)
      )
    );
    ...
    User.getData(id).then((data) => {
      ...
      if (chest.money < fraction.getAwardCount(data.fraction, data.rank))
        return player.notify(
          player.user.LangString("tablet.giveAward.data.money")
        );

      chest.money =
        chest.money - fraction.getAwardCount(data.fraction, data.rank);

      data.bank_money =
        data.bank_money + fraction.getAwardCount(data.fraction, data.rank);

      player.notify(
        player.user.LangString("tablet.giveAward.data.notify"),
        "success"
      );
      player.user.log(
        "tabletFraction",
        langStringDefault(
          "tablet.giveAward.data.log",
          data.id,
          data.fraction,
          data.rank
        )
      ...
      chest.addLog(
        player,
        langStringDefault(
          "tablet.giveAward.data.chestlog",
          system.timeStampString(system.timestamp, true),
          data.rp_name,
          fraction.getAwardCount(data.fraction, data.rank)
        )
      );
      data.save().then(() => {
        tablet.reloadFractionData(player);
