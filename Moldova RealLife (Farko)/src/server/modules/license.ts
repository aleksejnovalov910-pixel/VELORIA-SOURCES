...
const globalLicenses = ["truck", "air", "boat", "moto", "car"];
...
const openLicense = (player: PlayerMp, index: number) => {
    ...
    m.newItem({
        ...
        onpress: () => {
            ...
            if(isAnyLicenseNeedTheory){
                user.setGui('drivingschool');
                CustomEvent.triggerCef(player, 'drivingschool', getLicenses(player), globalLicenses.includes(cfg.license[0].id) ? 4 : 1, cfg.license[0].id)
                return;
            }
    ...
    m.newItem({
        ...
        onpress: () => {
            ...
            const isGlobalLicense = cfg.license.some(lic => globalLicenses.includes(lic.id));
            
            if (isGlobalLicense) {
                const globalLicenseCenters = [];
                LICENCE_CENTER_LIST.forEach((center, centerIndex) => {
                    center.license.forEach(license => {
                        if (globalLicenses.includes(license.id)) {
                            const existingCenter = globalLicenseCenters.find(c => c.index === centerIndex);
                            if (!existingCenter) {
                                globalLicenseCenters.push({
                                    index: centerIndex,
                                    licenses: [license]
                                });
                            } else {
                                existingCenter.licenses.push(license);
                            }
                        }
                    });
                });

                globalLicenseCenters.forEach(center => {
                    center.licenses.forEach(lic => {
                        submenu.newItem({
                            name: LicenseName[lic.id],
...
CustomEvent.registerCef('client:autoschool:selectCategory', (player: PlayerMp, category: string) => {
    ...
    if (index == -1 || licId == -1) return player.notify("Nu am gasit", "error");

    if (player.user.money < price) return player.notify("Nu ai banii", "error");
    if (!player.user.removeMoney(price, true, 'DrivingSchool')) return player.notify("Nu ai banii", "error");
...
const practiceResult = (player: PlayerMp, result: boolean) => {
    ...
    setTimeout(() => {
        ...
        const haveDoc = user.inventory.find(q =>
            q.advancedNumber === user.id &&
            q.advancedString === license.need_document &&
            q.item_id === 802 &&
            (!license.need_document_days ||
                (parseInt(q.serial.split('|')[1]) + license.need_document_days * 24 * 60 * 60) > system.timestamp)
        );

        if (license.need_document) {
            if (!haveDoc)
                return player.notify(
                    player.user.LangString("license.9e95026ceda06eced80b7011c0f48617",
                    document_templates.find(q => q.id === license.need_document).typeShort),
                    "error"
                );
        }

        if (!result)
            return player.notify(player.user.LangString("license.ae441f4674cb8ad1e9ad115c90641262"), "error"),
                player.dimension = 0;

        // 🔹 Scoasă plata, se acordă direct licența
        player.dimension = 0;
