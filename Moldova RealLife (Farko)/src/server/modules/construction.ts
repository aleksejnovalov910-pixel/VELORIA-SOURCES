...
CustomEvent.registerCef('flat:get', (player, id: number) => {
    ...
    if(!lobby) return player.notify(player.user.LangString("construction.cb93960feb0b9d8b79be08f8177598a2"), 'error');
    if(lobby.setsStatus.get(id)) return player.notify(player.user.LangString("construction.8623e030f8638d80e766cbe338ce6cb5"), 'error');
    if([...lobby.setsTaken].find(q => q[1] === user.id)) return player.notify(player.user.LangString("construction.bc608417e6a79569ffd2d52a9f860748"), 'error');
    if(lobby.setsTaken.has(id)) return player.notify(player.user.LangString("construction.045138c5d91510037af362eeffad7840"), 'error');
    if(lobby.ended) return player.notify(player.user.LangString("construction.cf09ba0d6b61db140e5764a71b194e57"), 'error');
    if(system.rebootStarter) {
        lobby.exit()
        return player.notify(player.user.LangString("construction.35b7192e8e5f6a8f7d00fc88c63f97c5"), 'error');
    }
    ...
    if(cfg.group){
        ...
        if(checkgroup){
            ...
            if(needList) return player.notify(player.user.LangString("construction.34e6096595b7abafc6e11aaff669146d", checkgroup), 'error')
        }
    }
...
CustomEvent.registerCef('flat:create', (player, pass: boolean) => {
    const user = player.user;
    if(!user) return;
    if(user.attachedToPlace) return player.notify(player.user.LangString("construction.5d298ea2a23d859402c255b11b08a3c7"), 'error')
    if(system.rebootStarter) return player.notify(player.user.LangString("construction.3307eb181e1b099c9c9996a7e2db6bee"), 'error');
...
CustomEvent.registerCef('flat:select', (player, id: number, pass?: string) => {
    const user = player.user;
    if(!user) return;
    if(user.attachedToPlace) return player.notify(player.user.LangString("construction.c3a8e9faaf2d9b0eb3f0d862f8677363"), 'error')
    const lobby = ConstructionSystem.get(id);
    if(!lobby) return player.notify(player.user.LangString("construction.67272b353946039734aba6c7e50b4251"));

    if(lobby.password){
        if(!pass) return player.notify(player.user.LangString("construction.0032765e5c34f6a3258b1ac4cd4787a1"));
        if(pass.toUpperCase() !== lobby.password.toUpperCase()) return player.notify(player.user.LangString("construction.0e72ffe594a39df0aa9a8a7709536389"), 'error');
    }
    if(lobby.players.length >= CONSTRUCTION_MAX) return player.notify(player.user.LangString("construction.22d8b58316c1e130efc8cc36b7abfafc"), 'error');
    if(system.rebootStarter) return player.notify(player.user.LangString("construction.dd4f8999469897ec98d24aa63800c760"), 'error');
    if(!ALLOW_JOIN_AFTER_START && lobby.started) return player.notify(player.user.LangString("construction.b79361e3f0a60f288abf1bbc82cd5112"), 'error')
...
CustomEvent.registerCef('flat:ready', (player, status: boolean) => {
    const user = player.user;
    if(!user) return;
    if(system.rebootStarter) return player.notify(player.user.LangString("construction.29cc913dbec9cf4f10911e7d32e27837"), 'error');
    const lobby = ConstructionSystem.getByPlayer(player);
...
const menuC = (player: PlayerMp) => {
    const user = player.user;
    if(!user) return;
    if(user.level < LEVEL_PERMISSIONS.ROOM) return player.notify(player.user.LangString("construction.e6f673900dfec897abafba828b21755f", LEVEL_PERMISSIONS.ROOM), 'error');
    ...
    const lobby = ConstructionSystem.getByPlayer(player);
    if(!lobby && user.attachedToPlace) return player.notify(player.user.LangString("construction.ffc462ff314c7c1b7dbdce9df749030d"), 'error');
...
colshapes.new(CONSTRUCTION_REGISTER_POS, player => player?.user?.LangString("construction.d6fe7492facf3a9078d51b3c8a06184c") ?? langStringDefault("construction.d6fe7492facf3a9078d51b3c8a06184c"), player => {
    menuC(player)
}, {
    type: 27,
...
export class ConstructionSystem {
    ...
    exit(){
        ...
        this.usersSetsCount.forEach((count, who) => {
            if(count > 0 && this.ended){
                ...
                if(player) {
                    player.user.addMoney(sum, true, player.user.LangString("construction.1bbe1048ed5a365062fa655486c58955"));
    ...
    start(){
        this.check()
        this.colshape = colshapes.new(new mp.Vector3(this.config.pos.x, this.config.pos.y, this.config.pos.z), player => player?.user?.LangString("construction.4bc8324c5ada50591af719977a863d5e") ?? langStringDefault("construction.4bc8324c5ada50591af719977a863d5e"), player => {
            const user = player.user;
            if(!user) return;
...
CustomEvent.registerClient('construction:set:ok', (player, id: number) => {
    ...
    if(!block) return player.notify(player.user.LangString("construction.1b14d17c975bf32e5da4e80fcff8c5ef"), 'error');
    const item = block.setsTaken.get(id)
    if(item !== user.id) return player.notify(player.user.LangString("construction.5c8d904ddf460e07aa54c13305bc5a85"), 'error');
    block.setSuccess(id, user.id);
    // mp.events.call(JOB_TASK_MANAGER_EVENT, player, 'apartmentRepair');
    player.notify(player.user.LangString("construction.5e897e5d656a14bdf94a8cf4efa48fe3"), 'success')
})