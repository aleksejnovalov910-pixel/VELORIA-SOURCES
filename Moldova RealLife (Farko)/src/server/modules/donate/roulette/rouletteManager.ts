...
export class RouletteManager {
    ...
    public static activateDrop(player: PlayerMp, dropId: number): void {
        ...
        player.user.log("DonateMoney", langStringDefault("rouletteManager.62add376e95adbdc6b892d8cddb50973", dropData.dropId));
        const drop = createDropByData(dropData);
        if (!drop.activate(player)) {
            player.notify(player.user.LangString("rouletteManager.29a4a58c681a990bc96be0e231b78260"))
            return;
        }
    ...
    public static addDrop(player: PlayerMp, dropId: number): void {
        player.user.log("DonateMoney", langStringDefault("rouletteManager.a0936aa3c50f049975cbfafb0aeb059c", dropId));
    ...
    public static sellDrop(player: PlayerMp, dropId: number): void {
        ...
        if (dropData.sellType === DropSellType.DOLLARS)
            player.user.addMoney(
                dropData.sellPrice, false, player.user.LangString("rouletteManager.fab074980f1d470fcaf3f331d6a039b4", player.dbid, dropData.dropId)
            );
        else player.user.addDonateMoney(
            dropData.sellPrice, player.user.LangString("rouletteManager.03140bbae0562a916f23239c68a0d90c", player.dbid, dropData.dropId)
        );
