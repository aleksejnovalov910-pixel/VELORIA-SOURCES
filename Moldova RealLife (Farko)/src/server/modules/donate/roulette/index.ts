...
gui.chat.registerCommand("setdrop", (player, targetIdStr, dropIdStr) => {
    ...
    if (target) {
        writeSpecialLog(langStringDefault("index.d98adab1943f90129f97ef995c7d09ed", dropId), player, target.user?.id)
...
CustomEvent.registerCef("droulette:request", (player: PlayerMp, type: string, count: number) => {
    ...
    if (player.user.donate_money < totalSpinCost) {
        player.notify(player.user.LangString("index.2a226e48d21237010db79cf0e30ae11f"), "error");
        return;
    }
    ...
    player.user.removeDonateMoney(totalSpinCost, player.user.LangString("index.afe907d7924fef0d79253083d19ce6f8"));
