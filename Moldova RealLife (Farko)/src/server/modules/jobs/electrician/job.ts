...
mp.events.add("vehicleDeath", (vehicle) => {
    ...
    player.notify(player.user.LangString("job.84f57ef7ceae2b871919166e82708ac3"));
})
...
CustomEvent.registerCef("electrician:check", (player: PlayerMp, type: WORK_TYPE) => {
    ...
    if (type === WORK_TYPE.houses || type === WORK_TYPE.large){
        if (!player.user.haveActiveLicense("car")) return player.notify(player.user.LangString("job.c29b3f6b61a82572acad197bc61bae49"));
...
CustomEvent.registerClient("electrician:payment", (player, index: string) => {
    ...
    if (amount === 0)
        writeSpecialLog(langStringDefault("job.32ea2d177f6c3aacfc1f4a3689e51e6d", index), player);

    player.user.addMoney(amount, false, player.user.LangString("job.c2e7d5009b3a177a3a17ef3cc43eb7bf"));
});