...
export class SanitationSquad {
    ...
    constructor(player: PlayerMp, isPublic: boolean, id: number) {
        ...
        if (this.password) player.outputChatBox(player.user.LangString("squad.7de18aac71869248b3a6699499c3e6cf", this.password));
    }
        ...
        if (step !== 5) {
            ...
            if (this.task.completedPoints.filter(el => el === true).length === 2) {
                ...
                this.players.forEach(target => {
                    ...
                    target.user.addMoney(400, true, target.user.LangString("squad.d0c65de2cc80a9091a9e115eb3998bf8"));
                })
            }
            ...
            if (!player.vehicle)
                return player.notify(player.user.LangString("squad.c7f251d1df5a708d2743f8cc3fb074b2"), "error");

            if (!player.vehicle.getVariable("sanitation") || player.vehicle.getVariable("sanitation") !== this.id)
                return player.notify(player.user.LangString("squad.e74479fca4d03632d28648b4f3488ab0"), "error");
