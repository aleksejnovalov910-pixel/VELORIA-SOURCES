...
CustomEvent.registerCef("busman:startWork", (player, type: number) => {
    if (!player.user.haveActiveLicense("truck")) return player.notify(player.user.LangString("index.dc3899414f79ce29a236e6e688a67357"));
...
CustomEvent.registerClient("busman:salary", (player, index) => {
    ...
    player.user.addMoney(amount, false, player.user.LangString("index.be18815a7470428e1e97bcd941fefea6"));
...
mp.events.add("vehicleDeath", (vehicle) => {
    ...
    player.notify(player.user.LangString("index.6ee5367ba2cc1f24724b503368cc9edb"));
})
