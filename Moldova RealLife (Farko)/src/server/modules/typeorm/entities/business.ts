...
import type { BUSINESS_TYPE } from "@/shared/business";
...
export class BusinessEntity extends BaseEntity {
	...
	get catalog(): {
		...
		const data = JSON.parse(this.catalog_data || "[]") as {
			item: number;
			price: number;
			count: number;
			max_count: number;
...
@Entity({ name: "business_history_entity" })
