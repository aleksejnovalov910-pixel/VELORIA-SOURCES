...
import {
	HOUSE_MAX_TAX_DAYS_DEFAULT,
	HOUSE_TAX_PERCENT,
	HOUSE_TAX_PERCENT_DAY_MAX,
} from "@/shared/economy";
...
import type { MiningHouseItemData } from "@/shared/mining";
...
export class HouseEntity extends BaseEntity {
	...
	garageAccessVehicle(veh: VehicleMp) {
		if (
			...
			veh.entity.familyOwner === this.familyId
		)
			return true;
		return (
			!this.forFamily &&
			veh.entity &&
			veh.entity.owner &&
			this.userList.includes(veh.entity.owner)
		);
	}

	get max_residents() {
		return MAX_RESIDENTS.find((q) => q.price <= this.price)
			? MAX_RESIDENTS.find((q) => q.price <= this.price).count || 1
			: 1;
	}
	...
	get taxDay() {
		return Math.min(
			(this.price / 100) * HOUSE_TAX_PERCENT,
			HOUSE_TAX_PERCENT_DAY_MAX,
		);
	}
	...
	removeMoneyChest(val: number, player: PlayerMp, reason: string) {
		if (player && reason && val)
			Logs.new(
				`housemoney_${this.id}`,
				`${player.user.name} ${player.user.id}`,
				`${reason} ($${system.numberFormat(val)})`,
			);
		if (this.forFamily) {
			const family = Family.getByID(this.familyId);
			if (family) {
	...
	set miningData(val) {
		...
		User.getNearestPlayersByCoord(
			new mp.Vector3(intData.mining.x, intData.mining.y, intData.mining.z),
