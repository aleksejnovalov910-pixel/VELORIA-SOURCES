...
export class GangwarGarage {
    ...
    public create(): void {
        ...
        this._colshape = colshapes.new(this._config.position, player => player?.user?.LangString("gangwar.garages.6d120ffa326398715df591e6edcc45a2") ?? langStringDefault("gangwar.garages.6d120ffa326398715df591e6edcc45a2"), player => {
    ...
    private openMenu(player: PlayerMp): void {
        const m = menu.new(player, "", player.user.LangString("gangwar.garages.387fc59ff78a3f9fc217137914883e9c"))
            ...
            m.newItem({
                ...
                onpress: () => {
                    ...
                    if (this._totalVehiclesTakenAmount >= this._vehiclesLimit) {
                        return player.notify(player.user.LangString("gangwar.garages.6ab11ea0d338754b40e569da46f50ffb"), "error")
                    }
    ...
    private spawnVehicle(player: PlayerMp, model: string): void {
        ...
        player.notify(player.user.LangString("gangwar.garages.6947a2bcaf965949066e875f8c28b5bc"), 'info')
    }
}