...
if (redisEnabled) {
  ...
  if (!redisHost) throw new Error('REDIS_HOST is required when REDIS_ENABLED=true');
...
console.log(`VELORIA host environment preflight: OK (server=${conf.name}, port=${conf.port}, slots=${conf.maxplayers}, redis=${redisEnabled ? 'enabled' : 'disabled'})`);
