...
function setWaypoint(xRaw:unknown,yRaw:unknown,labelRaw:unknown){const x=Number(xRaw),y=Number(yRaw),label=String(labelRaw??'Точка назначения').slice(0,80);if(!Number.isFinite(x)||!Number.isFinite(y))return false;mp.game.ui.setNewWaypoint(x,y);exec(`window.veloriaNotify?.('success',${JSON.stringify(`Маршрут построен: ${label}`)})`);return true}
...
mp.events.add('veloria:cef:property:lock',(propertyId:number,locked:boolean)=>mp.events.callRemote('veloria:property:lock',propertyId,locked));
mp.events.add('veloria:property:lockState',()=>mp.events.callRemote('veloria:tablet:data','property'));
...
mp.events.add('veloria:dealership:data',(dealershipId:number,json:string)=>{activeDealershipId=Math.max(1,Math.trunc(Number(dealershipId)||1));overlay('dealership',true,{dealershipId:activeDealershipId,stock:parsed(json,[])})});mp.events.add('veloria:cef:dealership:buy',(stockId:number)=>mp.events.callRemote('veloria:dealership:buy',stockId));mp.events.add('veloria:dealership:purchased',()=>mp.events.callRemote('veloria:dealership:stock',activeDealershipId));
