...
mp.events.add('veloria:cef:navigator:set',(xRaw:number,yRaw:number,nameRaw:string)=>{const x=Number(xRaw),y=Number(yRaw),name=String(nameRaw??'Точка назначения').slice(0,80);if(!Number.isFinite(x)||!Number.isFinite(y))return;mp.game.ui.setNewWaypoint(x,y);overlay('navigator',false);exec(`window.veloriaNotify?.('success',${JSON.stringify('Маршрут построен: ')}+${JSON.stringify(name)})`)});
