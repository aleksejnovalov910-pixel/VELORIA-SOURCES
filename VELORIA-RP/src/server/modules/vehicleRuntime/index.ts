...
function vec(raw: any, fallback: Vector3Mp): Vector3Mp {
  if (!raw || typeof raw !== 'object') return fallback;
  return new mp.Vector3(Number(raw.x ?? fallback.x), Number(raw.y ?? fallback.y), Number(raw.z ?? fallback.z));
}
...
async function garageSpawn(garageId: number) {
  ...
  let spawn: any = {};
  try { spawn = typeof row.spawn_json === 'string' ? JSON.parse(row.spawn_json) : (row.spawn_json ?? {}); } catch {}
  return spawn;
}
...
export function registerVehicleRuntimeModule(): void {
  ...
  mp.events.add('veloria:garage:take', async (player: PlayerMp, rawVehicleId: number, rawGarageId: number) => {
    const id = characterId(player); if (!id) return;
    const vehicleId = Number(rawVehicleId), garageId = Number(rawGarageId);
    try {
  ...
  mp.events.add('veloria:garage:park', async (player: PlayerMp, rawGarageId: number) => {
    const id = characterId(player); if (!id) return;
    const vehicle = player.vehicle; if (!vehicle) return;
