...
function parseAppearance(json: string): CharacterAppearance {
  try { return JSON.parse(json) as CharacterAppearance; }
  catch { return {} as CharacterAppearance; }
}
...
async function createCharacter(player: PlayerMp, slot: number, firstNameRaw: string, lastNameRaw: string, appearanceJson: string) {
  ...
  try { appearance = JSON.parse(appearanceJson) as CharacterAppearance; }
  catch { return player.call(Events.AuthResult, [false, 'Некорректные данные внешности']); }
...
async function selectCharacter(player: PlayerMp, characterId: number) {
  const id = accountId(player);
  if (!id || player.getVariable('veloria:authenticated') !== true) return;
  ...
  player.setVariable('veloria:characterId', character.id);
  (player as any).veloriaCharacterId = character.id;
  ...
  player.dimension = 0;
  player.call(Events.CharacterSpawned, [character.id]);
}

export function registerCharacterModule() {
  mp.events.add(Events.CharacterListRequest, (player: PlayerMp) => void sendCharacterList(player).catch((e) => logger.error('Character list failed', e)));
