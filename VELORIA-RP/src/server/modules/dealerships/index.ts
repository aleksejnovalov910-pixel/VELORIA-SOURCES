...
export function registerDealershipModule():void{
  mp.events.add('veloria:dealership:stock',async(player:PlayerMp,rawDealershipId:number)=>{
    ...
    try{
      ...
      player.call('veloria:notify',['error','Не удалось загрузить автосалон']);
    }
  });
  ...
  mp.events.add('veloria:dealership:buy',async(player:PlayerMp,rawStockId:number)=>{
    ...
    if(!id)return;
    try{
      const result=await buyDealershipVehicle(id,Math.trunc(Number(rawStockId)));
      ...
      const message=code==='OUT_OF_STOCK'?'Автомобиль закончился':code==='INSUFFICIENT_FUNDS'?'Недостаточно средств':'Покупка автомобиля не выполнена';
